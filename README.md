...
| Correzione assistita da IA (M5) | 🟡 Motore mock/UI M5-01→04 e infrastruttura server-side M5-05A→**E-1** implementati. HG-M5-1/2/3/4 approvati (modello/listino pinned, ceiling costi, retention), ma provider reale resta disabilitato: nessun secret, chiamata, costo, TTL reale o deploy; M5/G7 aperti. |
...
V1 comprende M1, M2, M3-lite, RE, QE e M3-full (tutti implementati, Gate G5 superato per M3-full). M4 è completato: correzione/restituzione, ciclo di vita, Registro Correzioni, export CSV ed export PDF (M4-00→M4-04, Markdown rinviato); **Gate G6 superato**. Il progetto può fermarsi dopo ogni modulo mantenendo un prodotto utile. M5 (correzione assistita da IA) ha motore mock/UI M5-01→04 e infrastruttura server-side M5-05A→E-1 implementati; provider reale, Secret Manager, TTL, smoke DEV e Gate G7 restano disabilitati/aperti. Didattica DUX-00→10A è implementata e il Gate GDUX è superato (PASS).
...
**M4 — Correzione ed export — completato (Gate G6 superato).** Il docente corregge, completa e restituisce; lo studente consulta la propria restituzione. La tabella Consegne online è il Registro Correzioni ed esporta CSV e PDF interamente nel browser (M4-00→M4-04; Markdown rinviato). Concept UX in [documentazione/m4-correzione-ux-concept.md](documentazione/m4-correzione-ux-concept.md); evidenze del gate in [documentazione/evidenze/g6-m4-checklist-finale.md](documentazione/evidenze/g6-m4-checklist-finale.md). **M5 — Correzione assistita da IA:** motore mock, UI e infrastruttura server-side fino a **M5-05E-1** sono implementati. Il docente ha approvato HG-M5-1/2/3/4 il 17 luglio 2026 ([evidenza](documentazione/evidenze/hg-m5-human-gate.md)); modello pinned, listino, ceiling per operazione/giorno/mese e retention sono formalizzati. L'approvazione non abilita il provider: nessuna API key, Secret Manager reale, chiamata, costo, policy TTL o deploy; M5 e Gate G7 restano aperti. Prossimo pacchetto: **M5-05E-2**.
...
**Hardening V1 — completato.** Gate GHARD superato: operatività/costi, regioni, security header, accessibilità e import resiliente verificati; residui P3 accettati con soglie. Per M5, motore mock/UI e infrastruttura fino a **M5-05E-1** sono implementati con provider reale disabilitato. Prossimo sviluppo: **M5-05E-2** (Secret Manager, TTL reale, config DEV col kill switch spento e primo smoke controllato), senza considerare superato G7 in anticipo.
