...
- il modulo IA (M5): due Function `onCall` `aiCorrectionPreview`/`aiCorrectionRun` (preview senza provider + esecuzione) usano un contratto **provider-agnostic** e contesto chiuso. **M5-05C** aggiunge `OpenAiGrader` e il benchmark harness dietro `disabled|mock|openai`, default `disabled`. **M5-05D1** impone sul percorso reale l'ordine auth/owner → config runtime/kill switch → classificazione e hard ceiling DEV → secret/grader → lease; `settings/aiConfig.model` è l'unica fonte del modello. Il preflight caricato viene riusato dopo la lease, mentre `commitSubmission` rilegge transazionalmente la correction contro le race. **M5-05D2A** rende `aiCorrectionRuns` server-only e privacy-minimal: selezione canonica + SHA-256, risultati persistiti solo per ordinale, replay ricostruito dalla selezione validata, legacy fail-safe ed `expireAt` DEV a 30 giorni senza policy TTL attiva. L'allowlist production (`gpt-5-nano-2025-08-07`) valida la coppia modello/listino. **M5-05D2B-1** collega al runtime costo e budget: costi stimati/effettivi in micro-USD interi (ripartizione input/output, `ceil`/`nearest`) e ledger mensile atomico `aiBudgetLedger/{YYYY-MM}`. La prenotazione è un **tetto conservativo** (massimo output del grader + upper bound provabile dell'input) distinto dalla stima informativa UI, con **macchina a stati crash-safe** (`reserved → pending`): una `reserved` scaduta è recuperabile, una `pending` scaduta è addebitata al tetto (mai sottocontabilizzare). Ordine sicuro lease → prenotazione → markBudgetInvoked → provider → commit → riconciliazione → finalizzazione (le ultime tre gated dalla titolarità della lease), **fail-closed** con `budget_unavailable` se porte/bounds mancano; mock e sole-chiuse a 0, nessun accesso al ledger. **M5-05D2B-2** aggiunge il retry applicativo **unico** del provider (SDK `maxRetries: 0`): ≤ 1 retry dalla config runtime su soli transitori, con `Retry-After` (cap oltre cui manuale), backoff esponenziale + jitter (`random`/`sleep`/clock iniettati, sleep annullabile), deadline complessiva monotona, `timeoutSeconds = 540` esplicito e `RUN_LEASE_MS` a 9 min coerenti; la prenotazione copre `bound × (retry+1)` e l'accounting è prudente sui tentativi incerti (`costActual ≤ costSettled ≤ costReservation`), con telemetria retry aggregata privacy-safe. Nessuno scheduler/polling; nessun secret reale, chiamata, costo, TTL o deploy; Human Gate/G7 restano aperti.
