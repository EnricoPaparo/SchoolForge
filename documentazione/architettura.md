...
- **Lezioni**: `publicLessons/{lessonId}` — solo `id`, `programId`, `udaId`, `title`, `order`, `contentPath` (percorso Storage del solo file lezione `.md`, mai del pool) e `validationStatus`. Le Storage Rules concedono lettura dei soli file lezione e asset dell'import attivo, mai dei file `.pool.md`.
...
In entrambi i casi, "agli utenti autenticati non-owner" della prima versione di questo ADR è stato sostituito dal gate di approvazione di ADR-14: la lettura richiede anche `students/{uid}.status == 'approved'` e `settings/studentAccess.studentPortalEnabled == true`, verificati dalle stesse Security Rules (Storage tramite `firestore.get()`/`firestore.exists()`).
...
**Decisione.** ADR-05 e ADR-06b (sopra) descrivono la risoluzione del *ruolo* (docente vs. studente) da `auth.uid` rispetto a `ownerUid`: questo distingue il proprietario da chiunque altro, ma non autorizza da solo alcuna lettura. Un utente Google non-owner è un *richiedente/studente potenziale* finché non viene esplicitamente approvato dal docente in `students/{uid}` (`status: "pending" | "approved" | "blocked"`), e finché il portale studente non è globalmente attivo in `settings/studentAccess.studentPortalEnabled`. Ogni lettura studente (`publicLessons`, `verifications/*/publishedProjection`, file lezione in Cloud Storage) richiede entrambe le condizioni contemporaneamente; l'assenza di uno dei due documenti nega per difetto, non richiede una migrazione o un caso speciale.

**Motivazione.** La prima versione di M3L-A (ADR-05/ADR-12/ADR-13) trattava "autenticato con Google, non-owner" come sufficiente per leggere le proiezioni pubbliche. Una revisione di sicurezza successiva ha stabilito che questo espone il Portale a chiunque possieda un account Google (personale o Workspace), non solo agli studenti effettivi del docente: un dominio Workspace condiviso con altri utenti, o un link della SPA girato per errore, avrebbe concesso lettura senza alcun controllo del docente. Introdurre un passaggio di approvazione esplicito, verificato dalle Security Rules a ogni lettura (non solo un controllo lato client), chiude questa esposizione senza introdurre Cloud Functions: sia Firestore sia Cloud Storage possono leggere `students/{uid}` e `settings/studentAccess` direttamente nelle regole (Storage tramite le funzioni cross-service `firestore.get()`/`firestore.exists()`).

**Conseguenza.** La UI di gestione studenti (creare/approvare/bloccare uno studente, assegnare una classe) non è ancora costruita — questa milestone consegna solo lo schema dati, le Security Rules e i test che le verificano; il docente non ha oggi un modo da interfaccia per popolare `students/{uid}`. La UI reale, l'assegnazione delle classi a programmi/verifiche e il filtro conseguente (vedi ADR-15) sono rinviati alla milestone successiva.

### ADR-15 — Filtro per classe di lezioni e verifiche (specifica futura, non implementata)

> Questa sezione descrive un'estensione pianificata ma non ancora implementata. Nessuno schema, Security Rule o UI descritti qui esiste nella baseline corrente.

**Decisione (rinviata).** Oltre al gate approved+portale attivo (ADR-14), un programma sarà visibile a uno studente solo se ha almeno una classe assegnata che include la classe dello studente (`students/{uid}.classId`); una verifica sarà visibile solo se ha un `classId` assegnato e coincidente. Un programma senza classi assegnate, o una verifica senza `classId`, non saranno mai visibili ad alcuno studente, anche se altrimenti pubblici.

**Motivazione (rinviata).** Un docente con più classi deve poter distribuire contenuti e verifiche diverse a classi diverse, senza dover creare account o portali separati.
...
| Cloud Storage | Markdown, asset; lettura dei soli file lezione (non pool) concessa allo studente **approvato**, con portale attivo, verificato via `firestore.get()`/`firestore.exists()` sulle regole. | Conservare PDF o export didattici; concedere lettura di file `.pool.md` allo studente; concedere lettura file lezione al solo utente autenticato non approvato. |
...
| `students/{uid}` (M3-lite) | `uid`, `ownerUid`, `email`, `displayName` (identità Google verificata da Firebase, non autodichiarata), `status` (`pending`/`approved`/`blocked`), `classId` (riservato al filtro futuro, ADR-15) | Registro di approvazione (ADR-14). Lettura e scrittura solo owner — non esiste ancora una UI docente per popolarlo, né un modo per lo studente di leggere il proprio stato. Assenza del documento equivale a `pending`. |
| `publicLessons/{lessonId}` (M3-lite) | `programId`, `udaId`, `title`, `order`, `contentPath` (solo file lezione), `validationStatus` | Proiezione read-only priva di riferimenti al pool; scritta dal docente nello stesso flusso di import. Lettura solo per uno studente approvato con portale attivo (ADR-14); scrittura solo owner. |
