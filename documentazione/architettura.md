...
- il modulo IA (M5): due Function `onCall` `aiCorrectionPreview`/`aiCorrectionRun` (preview senza provider + esecuzione) usano un contratto **provider-agnostic** e contesto chiuso. **M5-05C** aggiunge `OpenAiGrader` e il benchmark harness dietro `disabled|mock|openai`, default `disabled`; nessun modello/secret reale è configurato e nessuna chiamata esterna è stata eseguita. **M5-05D1** aggiunge i guardrail server-side (moduli puri `aiCorrectionRuntimeConfig`/`aiCorrectionLimits`/`aiCorrectionCost`/`aiCorrectionBudget`): config runtime fail-closed `settings/aiConfig` come **kill switch senza deploy** e limiti prudenziali DEV applicati nel preflight, collegati al motore **solo** sul percorso `openai`; listino prezzi versionato e ledger di budget restano moduli testati non ancora collegati. `MockAiGrader` resta disponibile e invariato; il provider reale richiede Human Gate/G7 e **non è ancora attivabile**.
