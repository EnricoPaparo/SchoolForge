...
**Motivazione.** Per una V1 single-docente Firebase riduce il lavoro di provisioning: hosting HTTPS, autenticazione, database, object storage, funzioni, emulatori e osservabilità sono integrati. Firestore è sufficiente ai flussi previsti, incluso il token mono-uso del tentativo digitale e il log di accesso nome+IP tramite transazioni.
...
- `startDigitalAttempt` (M3): brucia il token mono-uso, crea tentativo, snapshot con soluzioni private, voce di log accesso (nome+IP+user-agent+timestamp) e token opaco di sessione.
...
**Decisione.** Il link pubblico contiene un token casuale ad alta entropia associato a una verifica attiva. Lo studente dichiara nome e cognome; `startDigitalAttempt` brucia il token mono-uso, crea tentativo, snapshot e log di accesso (nome+IP+user-agent+timestamp) in transazione Firestore e restituisce un token opaco di ripresa come cookie sicuro.
...
| Cloud Functions | `startDigitalAttempt`: token mono-uso, snapshot con soluzioni private, log accesso, token sessione. M5/V2: chiamate AI con contesto chiuso e audit. | Generare PDF, inviare email, applicare stati verifica, scrivere Markdown. |
...
sequenceDiagram
    ...
    CF->>F: transazione — brucia token mono-uso, tentativo, snapshot con soluzioni private, accessLog (nome+IP), token sessione
...
|---|---|---|
| `startDigitalAttempt` | SPA portale | Transazione Firestore — brucia token mono-uso, tentativo, snapshot con soluzioni private, accessLog, token sessione. |
...
- Il token del tentativo digitale è mono-uso; ogni accesso è registrato con nome dichiarato, IP, user-agent e timestamp (Report Accessi).
...
| Integration | Emulator Suite: Security Rules, token mono-uso digitale, log accesso, import Storage, snapshot digitale. |
...
| Token mono-uso e audit | Token del tentativo digitale bruciato alla prima chiamata; log nome+IP+user-agent+timestamp nel Report Accessi. |
...
5. il token mono-uso impedisce una seconda consegna digitale e ogni accesso è registrato con nome+IP+timestamp;
