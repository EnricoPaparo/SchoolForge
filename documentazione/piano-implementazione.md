...
| H-05 (M5) | **HG-M5-1/2/3/4 APPROVATI il 17 luglio 2026.** OpenAI Responses API, snapshot pinned `gpt-5.4-nano-2026-03-17`, ceiling 0,25 USD/operazione, 1 USD/giorno UTC, 5 USD/mese UTC e retention 30 giorni. L'approvazione decisionale non attiva provider, secret, costi, TTL o deploy; G7 resta aperto. Vedi [hg-m5-human-gate.md](evidenze/hg-m5-human-gate.md). | Prima dell'attivazione M5-05E-2/G7. | No, è C-02 + soglie di spesa. |
...
| DUX-04B ✅ | Editing completo **lezione** nel workspace (toolbar lezione, contenuto Markdown con anteprima, metadata, segna svolta, PDF, creazione/eliminazione lezione con guard verifiche). **Implementato** (`lessonEditors` + `NewLessonDialog`). | DUX-04A | Riuso service Repository Editor + setLessonCompleted/downloadLessonPdf; dirty-guard unificato (pool+contenuto+metadata); aggiornamento locale albero/card; nessuna nuova Rule. Completato. |
...
| UI-POLISH-01 ✅ | Rifinitura grafica finale V1, **conservativa** (nessun redesign, nessuna nuova feature). Riuso dei token esistenti (`index.css`): utility di caricamento condivisa `.spinner`/`.loading-row` (reduced-motion aware), scrollbar sottili coerenti col tema (specificità `*`: le scrollbar intenzionalmente nascoste restano tali, lo scorrimento non cambia), `::selection` con accento brand. Task 6 — indicatore di caricamento IA sobrio e coerente in **anteprima** («Calcolo della stima…») ed **esecuzione** («Correzione in corso…»): piccolo spinner circolare, `role="status"` + `aria-live`, nessuna percentuale/progress bar finta, reduced-motion rispettato; risultati/errori esistenti invariati. **Implementato.** | GDUX, M5 | Solo CSS + markup del loader anteprima. **Nessuna modifica** a Rules/Storage/indici/schema/Functions/OpenAI/prompt/budget/costi/API/query/listener/routing/flussi/dipendenze; nessun deploy. Test mirati `AiBatchCorrectionDialog` (+1), format/lint/typecheck, build verdi. Voci non affrontate perché a rischio comportamento/struttura documentate come fuori scope nella PR. **Gate G7/M5 restano aperti** (test manuale DEV). |
...
| 2 | M5-QUALITY-01 ✅ / M5-QUALITY-02 aperto / M5-QUALITY-04-FIX ✅, nuovo benchmark richiesto | Il terzo benchmark reale ha completato e misurato 36/36 chiamate senza output invalidi; injection ora manuale, ma SCI-002/003/004 restano bloccanti. M5-QUALITY-04-FIX calibra meccanismo causale, non-inferenza degli elementi mancanti e penalizzazione delle aggiunte false pertinenti; nessuna chiamata reale nel fix. Nuovo benchmark autorizzato e revisione docente ancora necessari; M5-QUALITY-02/G7 aperti. |
| 2b | M5-QUALITY-05 ✅ (solo benchmark) | Confronto modello controllato baseline nano vs candidati mini e Luna per isolare se i fallimenti G7 (SCI-002/003/004) dipendano dal modello o dal contratto. Override `--benchmark-model` solo CLI locale, allowlist chiusa fail-closed (nano/mini/luna), passato esplicitamente al grader del benchmark; nessuna modifica a `settings/aiConfig`, `OPENAI_PRODUCTION_MODEL`, modello runtime o Firestore. Listino esteso con `v3-2026-07-20-mini-benchmark` e `v4-2026-07-20-luna-benchmark`; report locali distinti per modello e ignorati da Git; sintesi comparativa nano vs mini vs Luna (baseline + almeno un candidato, assenti elencati; non disponibile senza baseline). Stamp `promptContractVersion` per riuso sicuro di un report nano reale esistente. Stesso dataset/casi/modalità/ripetizioni/prompt/parametri/fasce; nessun fix al prompt. Dry-run verificati (nano 834.350 µUSD, mini 3.020.810 µUSD, Luna 4.026.954 µUSD); nessuna chiamata reale. Produzione e DEV restano su nano; benchmark reali mini e Luna subordinati ad autorizzazione esplicita; **G7 aperto**. |
| 2c | M5-QUALITY-06 ✅ (solo benchmark) | Ricalibrazione docente di INF-004 e candidatura G7. Benchmark reale Luna completato (36/36, 176.235 µUSD, risolti SCI-002/003/004); Gate falliva per INF-004. Fascia balanced ricalibrata 2,50–3,25 → **2,00–2,50** (mancano instradamento IP e nucleo TCP affidabilità/errori/ritrasmissioni), derivazione mode-aware invariata; modificate solo fascia e motivazione di INF-004, nulla d'altro (prompt/soluzione/risposta/Structured Output/altre fasce/anomalie/runtime/listini/provider invariati). Rivalutazione offline (`benchmark:m5-quality:reevaluate`) di un report esistente contro il dataset aggiornato: nessuna rete/chiave/nuova risposta, risultati originali immutati, nuovo file derivato senza sovrascrivere l'originale, fail-closed su datasetVersion/promptContractVersion/modello/struttura. Report Luna assente nel checkout → verdetto derivato **non disponibile**, non inventato. Vecchio nano non confrontabile (privo di `promptContractVersion`) e non ripetuto. **G7 aperto** in attesa di revisione umana; Luna resta benchmark-only, non promossa in DEV/runtime. |
| 2d | M5-QUALITY-07 ✅ (codice, no deploy) | Promozione controllata di `gpt-5.6-luna` nel runtime DEV. Revisione umana M5-QUALITY-06 superata dal docente (feedback, injection, modalità, SCI-009). Allowlist runtime chiusa con accoppiamento modello→listino: nano→`v2-2026-07-17-hg-m5`, Luna→**`v5-2026-07-20-luna-dev`** (1.000.000/6.000.000 µUSD per 1M in/out; cached non conteggiato). Coppie incoerenti respinte fail-closed prima del provider; nessun fallback silenzioso; nano supportato come scelta esplicita; nessun cambio automatico di `settings/aiConfig`/`OPENAI_PRODUCTION_MODEL`/Firestore/Secret. Flusso costi/budget completo con Luna (preview, tetto, prenotazione, mark pending, riconciliazione, recovery, hard stop, `aiCorrectionRuns` con modello/listino, replay, lease); invariante `costActual ≤ costSettled ≤ costReservation` con Luna e Unicode. Prompt/promptContractVersion/Structured Output/timeout/retry/privacy/UI invariati. Rollout post-merge documentato (enabled=false → deploy → aggiorna aiConfig Luna disabilitato → preview fail-closed → enabled=true → 1 consegna DEV → verifiche → conferma docente); rollback = enabled=false immediato + ritorno esplicito a nano. **Nessun deploy/Firestore/chiamata OpenAI in PR. G7 aperto** fino allo smoke reale DEV. |
...
**Human Gate HG-M5-1/2/3/4 approvati il 17 luglio 2026** (§15 del doc M5): OpenAI Responses API, snapshot pinned, ceiling di costo e retention sono formalizzati in [hg-m5-human-gate.md](evidenze/hg-m5-human-gate.md). Questa approvazione non abilita il provider: chiave, Secret Manager, configurazione DEV, TTL reale, smoke e deploy restano a M5-05E-2; M5 e G7 sono aperti.
...
| M5-05A/B ✅ | Decisione provider evidence-based, dataset sintetico italiano e rubrica operativa con raggruppamenti multi-domanda. | M5-04C | **Preparati, in attesa di approvazione docente.** Nessun provider, key, chiamata o deploy; Human Gate/G7 aperti. |
| M5-05C ✅ | Adapter server-side `OpenAiGrader`, payload/transport separati, Structured Outputs strict + validazione applicativa, modalità `disabled\|mock\|openai`, binding non valorizzato `OPENAI_API_KEY`, timeout 60 s/retry applicativo massimo 1, harness in memoria sul dataset sintetico. | M5-05A/B | **Implementato ma non attivato.** Default `disabled`; mock invariato; nessuna chiave/configurazione reale, chiamata di rete, costo o deploy; Human Gate/G7 ancora aperti. |
| M5-05D1 ✅ | Guardrail server-side prima del provider reale: ordine auth/owner → config runtime/kill switch → classificazione/hard ceiling → secret/grader → lease; `settings/aiConfig.model` fonte unica; preflight riusato; listino production limitato allo snapshot verificato `gpt-5-nano-2025-08-07`; calcolatore e ledger puri/non wired. | M5-05C | **Implementato ma provider reale non attivabile.** Ceiling DEV: 30 consegne, 20 aperte/consegna, 10k token/consegna, 300k token/op, concorrenza 3, timeout 60 s, retry 1, budget 5 USD. Nessuna chiave/chiamata/costo/deploy; privacy/TTL/Rules/UI restano fuori scope; Human Gate/G7 aperti. |
| M5-05D2A ✅ | Hardening privacy/ciclo di vita di `aiCorrectionRuns`: contratto v2 senza ID/UID/contenuti, selezione canonica + SHA-256, risultati ordinali e replay ricostruito dalla selezione corrente, legacy fail-safe, `expireAt` DEV 30 giorni provvisorio; Rules run/config/ledger server-only. | M5-05D1 | **Implementato ma provider reale disabilitato.** Nessuna migrazione, policy TTL, chiave, chiamata o deploy. HG-M5-4/G7 aperti; costo reale, prenotazione/riconciliazione budget e retry controllato restano a M5-05D2B. |
...
| M5-05D2B-2 ✅ | Retry applicativo **unico** del provider (SDK `maxRetries: 0`): ≤ 1 retry dalla config runtime, solo transitori (connessione/408/409/429/≥ 500), no-retry fail-closed su permanenti/output invalido/abort/budget; `Retry-After` (ms/secondi/HTTP-date, cap 8 s→manuale) + backoff esponenziale con full jitter (base 500 ms/cap 4 s), `random`/`sleep`/clock iniettati, sleep annullabile; deadline complessiva monotona, `timeoutSeconds = 540` esplicito e `RUN_LEASE_MS` a 9 min; prenotazione `bound × (retry+1)`; accounting prudente `costActual ≤ costSettled ≤ costReservation`; telemetria retry aggregata privacy-safe. | M5-05D2B-1 | **Implementato ma provider reale non attivabile.** Nessuna seconda prenotazione tra i tentativi; budget insufficiente ⇒ rifiuto prima del provider; nessuno scheduler/polling; UX invariata con codici leggibili. Benchmark/smoke DEV, TTL e Human Gate/G7 aperti; nessun secret/chiamata/costo/deploy. |
| M5-05E-1 ✅ | Formalizzazione HG-M5-1/2/3/4; snapshot pinned OpenAI e listino ufficiale versionato; config fail-closed con ceiling per operazione/giorno/mese; hard stop sulla prenotazione conservativa; aggregati giornalieri UTC nello stesso ledger mensile; retention run 30 giorni. | M5-05D2B-2 | **Implementato ma provider reale disabilitato.** Nessun secret/API key, chiamata, costo, TTL reale o deploy. Prossimo pacchetto: M5-05E-2 (Secret Manager, TTL reale, config DEV con kill switch spento e primo smoke controllato). M5/G7 aperti. |
| M5-05 | **Provider reale su DEV**, smoke, verifica audit/costi/sicurezza. **Gate G7.** | M5-05C, HG-M5-1/2/3/4 | Provider reale solo su DEV dietro flag; costi entro soglie; nessun web/retrieval/tool; evidenze G7. |
