...
| 5b | ANNOT-CLEANUP-01 ✅ | Eliminazione sicura ed economica degli appunti personali degli studenti alla cancellazione del corso. Cloud Function `onCall` owner-only `cleanupProgramLessonNotes` (Admin SDK, region `us-central1`, scale-to-zero): una collection-group query su `lessonNoteIndexes` per `programId`, path note costruiti da `studentUid` + `lessonIds` (i `lessonNotes` non sono mai letti → il docente non legge i contenuti), validazione fail-closed, delete in chunk di 400 (prima note poi indici), idempotente. Invocata da `deleteProgram` prima della delete di `programs/{programId}`; se fallisce il corso resta per il retry. Copre le note tracciate dall'indice ANNOT-03B, nessun fallback di scansione né migrazione legacy; nessun indice composito/scheduler/polling/TTL. Costo solo alla cancellazione: `S` read indice + `N` delete note + `S` delete indice. Rules appunti invariate. |
