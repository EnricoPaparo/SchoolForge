...
| POOL-TUNE-02-CANDIDATE-A ✅ | Correzione mirata del prompt pool sui difetti misurati nel profile probe. | POOL-TUNE-01 | **Implementato e verificato staticamente; lotto reale non eseguito.** Versione `pool-tune-02-candidate-a-v1`: indici zero-based e audit finale delle opzioni, matrice privata anti-duplicazione anche fra tipi diversi, vere interruzioni di riga e divieto di `\\n`/`\\r`/`\\t` letterali. Schema, payload, cap, profilo runtime, costi e backend invariati. Dry-run Quality: 8 chiamate, fino a 16 tentativi, stima 216.257 µUSD, tetto 676.954 µUSD, zero rete/secret. Evidenza: `evidenze/pool-tune-02-candidate-a-review.md`. Review reale, holdout e Gate GPOOL-QUALITY aperti. |
