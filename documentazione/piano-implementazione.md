...
| HARD-01C ⏳ | Region e residenza dati (F02): matrice regioni DEV/PROD (`evidenze/hard-01c-region-matrix.md`) e riconciliazione documentale — corrette le affermazioni assolute «tutto a Milano `europe-west8`» in README/architettura/sicurezza con lo stato reale (DEV Storage/Function `us-central1` verificato; Firestore DEV **NON VERIFICATA**; target PROD **UE**). **F02 = MITIGATED — contraddizione eliminata; blocker PROD (decisione regione UE + provisioning ex novo) e verifica Firestore DEV aperti** (`evidenze/hard-01c-human-gate.md`). Audit read-only: nessun servizio creato, nessun dato migrato, nessun deploy. Nessuna modifica a codice/Rules/`firebase.json`/`.firebaserc`/schema/indici/dipendenze. | HARD-00 | **Doc riconciliata; blocker PROD + verifica Firestore DEV pending.** |
