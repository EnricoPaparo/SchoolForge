...
| LESSON-TUNE-MODEL-01 📐 | Confronto controllato sul candidato D: stesso prompt, dataset, otto casi tuning, payload e token cap; unica variabile `economy` vs `quality`. | LESSON-TUNE-01 | D economy respinto per blocker IPv4 e trasferimento termico. Runner quality abilitato solo sul tuning, mapping server-side Luna/listino v5, frase dedicata, output separati. Dry-run: 8 chiamate, ≤16 tentativi, stima 451.904 µUSD, tetto 1.070.842 µUSD; zero secret/rete. Holdout protetti. Evidenze in [evidenze/lesson-tune-05-candidate-d-review.md](evidenze/lesson-tune-05-candidate-d-review.md). |
