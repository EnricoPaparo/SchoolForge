...
| HARD-02B-2 | Chunking/resilienza (F06): scritture tecniche in chunk ≤400, macchina a stati `staging→active→superseded`, switch atomico su `activeImportId`, cleanup differito delle **sole** `publicLessons` (mai dati tecnici/Storage) con risultato `committed`+`cleanupPending`. Ordine rollout indice→Rules→Hosting→smoke. Chiude HARD-F06. | HARD-02B-1 | Non implementato. |
