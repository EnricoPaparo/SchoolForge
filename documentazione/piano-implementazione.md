...
| VISUAL-ENRICHMENT-05A | **Infrastruttura benchmark locale.** Dataset 8 tuning + 4 holdout con SHA-256, rubriche 6+10, blocker, runner dry-run protetto, provider iniettati, checkpoint/resume e checklist layout/rollout. | VE-04 | **Implementato; primo tuning reale eseguito e fermato da blocker del prompt.** Sette proposte su otto superavano il limite del `subject`; zero immagini generate. Correzione candidata `visual-proposal-01-v2`; nuovo tuning richiesto prima dell'holdout. Rollout non eseguito. Evidenza: [visual-enrichment-05a-benchmark.md](evidenze/visual-enrichment-05a-benchmark.md). |
