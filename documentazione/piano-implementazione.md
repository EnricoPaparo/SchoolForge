...
|---|---|
| Prerequisiti | M4-02 |
| File creati | `apps/web/src/features/student/studentCorrectionReturnsService.ts` — `loadStudentCorrectionReturns` (query `correctionReturns` filtrata `studentUid == uid` + `visibleToStudent == true`, ordinata `returnedAt desc`, un'unica lettura server-side) e `loadStudentCorrectionReturn` (singolo `getDoc` per il pulsante "Ricarica", denial → `null`, mai un'eccezione non gestita); `apps/web/src/features/student/StudentCorrectionView.tsx` (+ `.module.css`) — workspace read-only, riprende il linguaggio visivo di `OnlineExamView`/`CorrectionWorkspace` (pannello sticky, navigatore domande) letto esclusivamente da `correctionReturns` (mai `corrections`, `correctionEvents`, la submission consegnata o `teacherSnapshot`); test dedicati (`studentCorrectionReturnsService.test.ts`, 8 test; `StudentCorrectionView.test.tsx`, 13 test) |
| File modificati | `apps/web/src/features/student/StudentVerificationsView.tsx` (nuovo stato `correctionReturns` caricato una volta insieme alla lista; lista suddivisa in tre sezioni — "Correzioni restituite"/"Consegne effettuate"/"Verifiche disponibili" — pulsante "Vedi correzione" mostrato solo quando esiste una restituzione visibile caricata; nuovo `view.mode === 'correction'`), `.module.css` (stili gruppo/pulsante), `firestore.indexes.json` (nuovo indice composito `COLLECTION` su `correctionReturns`: `studentUid` ASC, `visibleToStudent` ASC, `returnedAt` DESC), `apps/web/src/rules/m4-01-corrections.rules.test.ts` (+5 test mirati sulla query, nessuna modifica a `firestore.rules`) |
| Flusso | Al caricamento della lista, `loadStudentCorrectionReturns` gira in parallelo ai controlli di stato online esistenti → le verifiche con restituzione visibile finiscono nella sezione "Correzioni restituite" con pulsante "Vedi correzione" → click apre `StudentCorrectionView` con i dati già caricati (nessuna lettura aggiuntiva all'apertura) → navigazione tra domande in sola lettura, punteggio/feedback per domanda, soluzione solo se `solutionsVisible == true` **e** `correctAnswer` è presente su quella domanda → "Ricarica" manuale rilegge il singolo documento (utile se il docente ha appena restituito un aggiornamento o nascosto il risultato) → "Torna alle verifiche" chiude senza ricaricare l'intera lista. |
| Sicurezza | Nessuna modifica a `firestore.rules`: la regola `allow read` di `correctionReturns` già introdotta in M4-01 (`studentUid == request.auth.uid && visibleToStudent == true`) autorizza sia il `getDoc` singolo sia la nuova query `list`, verificato con Emulator (`m4-01-corrections.rules.test.ts`, +5 test: query con entrambi i filtri ammessa; query priva di un filtro negata, incluso il caso "solo `studentUid`" che esporrebbe anche le restituzioni nascoste; nessuna fuga di dati di un altro studente). Aggiunto solo l'indice composito necessario alla query (`firestore.indexes.json`), nessun indice ridondante. |
...
| Test minimi | Query con i filtri e l'ordinamento corretti; un solo `getDocs`, mai un `getDoc` per verifica; normalizzazione con `submissionId` come id documento; ordinamento per `returnedAt` decrescente con re-sort difensivo in JS; nessun crash su `returnedAt` legacy/mancante; `loadStudentCorrectionReturn` per singolo id, `null` su "non esiste" e su "lettura negata" (mai un'eccezione propagata); componente: header (titolo/classe/date/totale/percentuale/feedback generale), navigatore domande, contenuto per domanda (testo/opzioni/risposta consegnata/punteggio/feedback docente), nessun colore che suggerisca correttezza sulle opzioni, soluzione assente per default e presente solo quando `solutionsVisible` e `correctAnswer` coesistono, "Ricarica" che aggiorna/mostra stato non disponibile/errore leggibile, "Torna alle verifiche"; vista lista: badge "Vedi correzione" solo per restituzioni caricate e visibili, sezioni distinte, apertura/chiusura del workspace, tolleranza a un fallimento del caricamento delle restituzioni. |
| Evidenza richiesta | `pnpm format:check`, `pnpm lint`, `pnpm --filter web exec tsc --noEmit`, test mirati (`studentCorrectionReturnsService.test.ts` 8, `StudentCorrectionView.test.tsx` 13, `StudentVerificationsView.test.tsx` 31 incl. i 6 nuovi, `studentVerificationsService.test.ts` 11 come sanity check) e `m4-01-corrections.rules.test.ts` (73 test, solo il nuovo caso query) verdi via Emulator; `pnpm build` verde. Nessun `pnpm test:rules` completo (nessuna modifica a `firestore.rules`). Nessun deploy. |
