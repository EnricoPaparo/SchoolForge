...
| VISUAL-ENRICHMENT-05A | **Infrastruttura benchmark locale.** Dataset 8 tuning + 4 holdout con SHA-256, rubriche 6+10, blocker, runner dry-run protetto, provider iniettati, checkpoint/resume e checklist layout/rollout. | VE-04 | **Implementato; due tuning reali eseguiti.** Il primo ha rilevato il limite non comunicato del `subject`; il secondo ha prodotto 8/8 proposte valide e 6 immagini tecnicamente valide, ma la review ha bloccato il rollout per testo, relazioni e istruzioni inventate. Candidato `schoolforge-sketch-prompt/v2` con allowlist testuale e grounding stretto; nuovo tuning richiesto prima dell'holdout. Rollout non eseguito. Evidenza: [visual-enrichment-05a-benchmark.md](evidenze/visual-enrichment-05a-benchmark.md). |
