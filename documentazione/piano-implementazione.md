...
| VDIF-01 ✅ | **Registro etichette owner-only**: `DifferentiationLabelDoc` a **otto chiavi** (`labelId`, `ownerUid`, `name`, `nameKey`, `assignedCount`, `draftUsageCount`, `createdAt`, `updatedAt`), prenotazione `differentiationLabelNames/{SHA-256(ownerUid + U+0000 + nameKey)}` a quattro chiavi, helper puri `labelName.ts` (trim, collasso spazi Unicode, limiti 40 code point / 120 byte, no caratteri di controllo) e `labelReservationId.ts` (Web Crypto, fail-closed), service canonico con **CRUD interamente transazionale** e audit nello stesso commit, Rules owner-only a contratto chiuso su entrambe le collezioni, **terza scheda «Etichette»** (tablist a tre schede con frecce cicliche/Home/End/roving tabindex, pulsante full-width, card full-width, dialog crea/rinomina/elimina, elimina protetta dai due contatori con motivo via `aria-describedby`, stato vuoto senza esempi diagnostici). | VDIF-00 | **Implementato, non distribuito.** Suite web 3.046, Rules 606, typecheck/lint/build verdi. Nessun indice nuovo, nessuna lettura per card, nessun listener/polling, nessuna Function. `functions/` invariata. Idempotenza limitata alla transazione (nessun `requestId` in contratto): limite dichiarato in `verifiche-differenziate-roadmap.md` §18.3. **Gate GVDIF aperto.** |
| VDIF-02 ✅ | **Assegnazione privata studente → etichetta**: `studentLabelAssignments/{studentUid}` owner-only a **cinque chiavi** con id deterministico (cardinalità garantita dal path), service canonico **interamente transazionale** con contatori `assignedCount` espliciti (mai `increment` alla cieca, mai `max(0, n − 1)`) e audit `student.labelAssigned` nello stesso commit, `removeStudent` promosso a transazione che elimina studente e assegnazione e rilascia il contatore insieme, Rules owner-only a contratto chiuso con integrità referenziale **finale** su etichetta e studente (`existsAfter`/`getAfter`), controllo «Etichetta» accanto a «Classe» nella card studente (salvataggio immediato, busy e guardia anti-doppio-click per singola card, errore ancorato con scelta tentata e retry esplicito), ricerca estesa al nome dell'etichetta e a «Nessuna etichetta». | VDIF-01 | **Implementato, non distribuito.** Una sola query di assegnazioni per caricamento, **zero letture per card**, **zero refetch** dopo una mutazione (contatori autorevoli restituiti dalla transazione). Card diverse possono salvare in concorrenza senza sbloccarsi a vicenda. Nessun indice nuovo, nessun listener/polling, nessuna Function; `functions/` invariata. Privacy verificata da un test **strutturale** sulle fonti, non sull'interfaccia. **Gate GVDIF aperto.** |
| VDIF-03 ✅ | **Builder delle varianti**: helper puro condiviso `classifyQuestionParticipation` (`common_free`/`vex_member`/`differentiated_base`/`differentiated_alternative`), pulsante «Varianti (n)» su ogni domanda comune e **disabilitato con motivo** sulle domande VEX, dialog a tre valori per etichetta con alternative filtrate (stessa lezione, non selezionate, non VEX, senza duplicazioni) e anteprima reale, riuso di `VexQuestionSelect`, `config.differentiation` versionata nello **stesso** «Salva bozza», **mutua esclusione VEX bidirezionale** senza conversioni automatiche. `updateVerificationConfig` è transazionale: diff di insiemi sui `labelId`, contatori `draftUsageCount` espliciti, audit nello stesso commit e replay no-op. | VDIF-02 | **Implementato, non distribuito.** Una query etichette insieme all'apertura della bozza; zero letture/scritture all'apertura del dialog; quando l'insieme etichette non cambia, zero letture/scritture sulle etichette. La soluzione completa resta nel pool owner-only e non viene caricata nel builder, che usa la preview sicura già presente nel `questionIndex`. Una bozza differenziata è salvabile ma l'attivazione resta fail-closed fino a VDIF-04, per non perdere silenziosamente le varianti. **Gate GVDIF aperto.** |
| VDIF-04 ✅ | **Attivazione reale**: guardie G01→G21 nell'ordine congelato (G20 immediatamente prima della transazione, dove una `getDocs` è ammessa), snapshot privato autosufficiente (`differentiation` con `labels[]` = `labelId` + `labelName` congelati, `labelAssignments`, `commonQuestionOrders` anche in `same_questions`), `questions[]` come **unione** di selezionate e alternative lette in **una sola** chiamata Storage, `resolveDifferentiatedOrders` autorevole nelle Functions con vettori di conformità condivisi, `assignedQuestionOrders` prodotti dalla callable **esistente** `assignVerificationVariant`, **`assignmentMode` neutro** come unico campo nuovo della proiezione, decremento di `draftUsageCount` **nello stesso commit** di stato/snapshot/proiezione, riepilogo pre-attivazione owner-only con blocker per percorso. | VDIF-03 | **Implementato, non distribuito.** Suite web 3.388, Functions 1.098, Rules 640 (invariate: la proiezione non ha un contratto chiuso di chiavi), typecheck/lint/format/build verdi. Nessuna nuova collezione, callable, Function, dipendenza o indice; zero letture per alternativa, per studente o per etichetta a regime. Smoke responsive a 1440/1024/390/320 in [evidenze/vdif-04-smoke.md](evidenze/vdif-04-smoke.md). **VDIF-05 e Gate GVDIF aperti.** |
| VDIF-05 ✅ | **Consumer downstream**: svolgimento studente, correzione manuale, correzione IA, restituzione, PDF, CSV, ricevute e **privacy audit** end-to-end sull'elenco chiuso delle superfici. | VDIF-04 | **Implementato.** `same_questions` con differenziazione è server-resolved in tutti i consumer; correzione manuale/IA, restituzione e archivio usano soltanto gli order assegnati; il PDF studente usa `resolveStudentVerificationPdf`, riusa la stessa assegnazione dello svolgimento e non crea una submission; numerazione locale densa; errori studente opachi; nessun metadato etichetta nei contratti studente, ricevute, export o run IA. Evidenze in [`evidenze/vdif-05-consumer-audit.md`](evidenze/vdif-05-consumer-audit.md). **Gate GVDIF aperto.** |
