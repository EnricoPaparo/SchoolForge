...
| 6 | VEX-00 ✅ / VEX-00B ✅ (docs+prototipo) / VEX-01A ✅ (client) / VEX-01B ✅ (server) / VEX-02A ✅ (studente) / VEX-02B ✅ (correzione) / VEX-02C ✅ (builder assistito) / VEX-03 | Gruppi equivalenti, assegnazione server-side persistente e varianti sicure senza esporre alternative. VEX-00B congela il contratto tecnico ([`vex-contract.md`](vex-contract.md)) e produce il prototipo statico del builder ([`prototipi/vex-builder.html`](prototipi/vex-builder.html)) — solo documentazione, nessun codice/Function/Rule/indice/schema/dipendenza. VEX-01A ✅ (client): modello dati + builder draft-time (rimuove `questionsPerStudent` inutilizzato, assorbito) + helper puri (normalizzazione modalità fail-closed, riconciliazione gruppi, conversione entryId→order) + **guardia fail-closed**: `activateVerification` rifiuta `equivalent_variants` finché VEX-01B non fornisce callable+isolamento — VEX **non operativo**, nessuna Function/Rule/indice/schema. VEX-01B ✅ (server): attivazione `equivalent_variants` operativa (snapshot VEX + proiezione **solo comuni**), callable **`assignVerificationVariant`** (una alternativa per gruppo, RNG `node:crypto` sicuro e iniettabile, transazione idempotente, **unica** scrittura `assignedQuestionOrders`, 0 al riaccesso), isolamento alternative + Rules server-only; guardia VEX-01A rimossa. VEX-02A ✅ (studente): routing fail-closed su `distributionMode`, svolgimento della variante assegnata via callable idempotente (avvio/ripresa/refresh), `OnlineExamView` solo variante, autosave/consegna ristretti (client fail-closed + Rules `answers`/`flagged` ⊆ `assignedAnswerKeys`), PDF studente disabilitato in VEX; `same_questions` invariato. VEX-02B ✅ (correzione): risolutore canonico `resolveAssignedQuestions` (unica fonte di verità), modalità ricavata solo da `teacherSnapshot.distributionMode`, scheletro correzione + totali + restituzione + payload IA sulla sola variante; IA esclude le consegne a variante malformata (`invalid_variant`) prima di grader/budget; registro PDF/CSV su totali variante; PDF docente completo invariato; nessun nuovo listener/query/documento. VEX-02C ✅ (builder assistito): precompilazione **tecnica** dei gruppi (UDA+tipo+difficoltà, `vexAutogroup.ts`, fail-closed) alla prima transizione a `equivalent_variants` + abbinamento progressivo delle nuove selezioni; **modifiche manuali autorevoli** (nessun ricalcolo globale); avviso permanente «tecnica, non pedagogica»; selettore leggibile (listbox accessibile a due righe con `questionPreview`, mai persistita). Puramente client, **zero costo/backend**; `same_questions` invariato. VEX-03: rollout coordinato + hardening equità/costi/smoke + Gate GVEX (**PENDING**). |
