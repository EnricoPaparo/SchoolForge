...
| VISUAL-ENRICHMENT-05A | **Infrastruttura benchmark locale.** Dataset 8 tuning + 4 holdout con SHA-256, rubriche 6+10, blocker, runner dry-run protetto, provider iniettati, checkpoint/resume e checklist layout/rollout. | VE-04 | **Implementato; tre tuning reali eseguiti.** Il terzo ha validato grounding e resa delle 6 immagini generate, con un solo rifiuto dovuto a `##` aggiunto all'heading di ancoraggio. Candidato `visual-proposal-01-v3`: `anchorHeadingText` è ora un enum request-specific degli H2/H3 reali; ultimo tuning richiesto prima dell'holdout. Rollout non eseguito. Evidenza: [visual-enrichment-05a-benchmark.md](evidenze/visual-enrichment-05a-benchmark.md). |
