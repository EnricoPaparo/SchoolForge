...
| UI-VERIFICHE-06B ✅ | `verificationDate` obbligatoria alla creazione e modificabile in bozza (validazione pura e rigorosa, nessuna normalizzazione, nessun Timestamp); `topicOutline` mantenuto nella stessa scrittura della bozza e ricostruito autorevolmente all'attivazione; card docente a tre riquadri e card studente allineate, con popup Argomenti condivisa. **Implementato.** | UI-VERIFICHE-06A | Nessuna Rule, nessun indice, nessuna migrazione, nessun documento Firestore aggiuntivo: due letture in più solo nel percorso di attivazione e nell'apertura di una bozza (insieme a quelle già necessarie). Costo a regime invariato; VEX, correzione, PDF, online e menu Azioni intatti. |
