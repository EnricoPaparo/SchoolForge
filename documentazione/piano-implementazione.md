...
| 2b | M5-QUALITY-05 ✅ (solo benchmark) | Confronto modello controllato baseline nano vs candidato mini per isolare se i fallimenti G7 (SCI-002/003/004) dipendano dal modello o dal contratto. Override `--benchmark-model` solo CLI locale, allowlist chiusa fail-closed (nano/mini), passato esplicitamente al grader del benchmark; nessuna modifica a `settings/aiConfig`, `OPENAI_PRODUCTION_MODEL`, modello runtime o Firestore. Listino esteso con nuova versione `v3-2026-07-20-mini-benchmark`; report locali distinti per modello e ignorati da Git; sintesi comparativa solo con entrambi i report (altrimenti non disponibile). Stesso dataset/casi/modalità/ripetizioni/prompt/parametri/fasce; nessun fix al prompt. Dry-run verificati (nano 834.350 µUSD, mini 3.020.810 µUSD); nessuna chiamata reale. Produzione e DEV restano su nano; benchmark reale mini subordinato ad autorizzazione esplicita; **G7 aperto**. |
