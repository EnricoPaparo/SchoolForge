...
| LESSON-TUNE-01 ✅ | Benchmark esteso anti-overfitting: i 6 scenari storici restano immutabili e sono combinati con 6 nuovi casi interdisciplinari; split congelato 8 `tuning` + 4 `holdout`, versione del prompt registrata nei report, CLI separata fail-closed e protocollo che vieta di usare gli holdout per ritoccare il candidato già esaminato. | LESSON-MANUAL-03 | **Baseline tuning reale completata:** 8/8 chiamate economy, 33.237 µUSD (0,033237 USD), zero Firestore/Storage e nessun billing risk. Review: **REVISIONE_SOSTANZIALE** per LaTeX incompatibile col renderer, struttura meccanica, precisione epistemica e refusi ricorrenti. Candidato A `lesson-tune-01-candidate-a-v1` implementato e verificato con Node 22.23.1 (Functions 837/837, build Functions/web, dry-run: 8 chiamate, ≤16 tentativi, tetto 212.588 µUSD), senza cambiare modello/listino/payload/token cap. I 4 holdout restano intatti; nuova esecuzione tuning solo dopo autorizzazione. Evidenze in [evidenze/lesson-tune-01-baseline-review.md](evidenze/lesson-tune-01-baseline-review.md). |
