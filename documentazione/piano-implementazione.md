...
Scope preciso per il prossimo pacchetto, derivato direttamente dal contratto M4-00 (nessuna decisione contrattuale residua bloccante: modello eventi e proiezione studente sono entrambi definiti):

| Campo | Valore |
|---|---|
| Prerequisiti | M4-00 |
| File da creare | `correctionsService.ts` (client-only: `openOrLoadCorrection`, `saveEvaluations`, `completeCorrection`, `returnCorrection`, `reopenCorrection`, `setSolutionsVisible`, `setReturnVisibleToStudent`), test service con Emulator Suite; blocco `corrections`/`correctionEvents`/`correctionReturns` in `firestore.rules`, test Rules dedicati |
| File da modificare | Nessuno applicativo oltre ai nuovi file — nessuna UI in questo pacchetto |
| Scope service layer | `openOrLoadCorrection(submissionId)`: legge `corrections/{submissionId}`; se assente, lo crea a `status:'in_progress'`, `reopenCount: 0`, con `evaluations` inizializzate da `publishedProjection.questions` (`order`, `maxPoints`, `points: null`), in un'unica scrittura. `saveEvaluations(...)`: aggiorna `evaluations`/`generalFeedback` e i totali derivati (`computeCorrectionTotals`) con una singola `update`; usa `assertValidQuestionPoints` prima di scrivere, mai un clamp silenzioso; se `isReopenedCorrection(correction)`, calcola `computeQuestionEvaluationDeltas`/`computeGeneralFeedbackDelta` **prima** di scrivere e, solo se producono almeno un elemento, appende un `correctionEvents` (`type:'scoreAdjusted'`) nella stessa `writeBatch` dell'update — se `reopenCount === 0` non scrive mai un evento, indipendentemente da quanti salvataggi avvengano. `completeCorrection(...)`: usa `assertValidCorrectionStatusTransition`, richiede `isCorrectionComplete`, imposta `completedAt`. `returnCorrection(...)`: transizione `completed→returned`, scrive/riscrive `correctionReturns/{submissionId}` (con `questions[]` autosufficiente copiato da `submissions.answers` + `publishedProjection.questions`, `visibleToStudent: true`, `solutionsVisible: false`) nella stessa `writeBatch` di update+evento `'returned'` (atomico, stesso pattern di `submissionsService`/`setVerificationVisibility`), imposta `returnedAt`. `reopenCorrection(...)`: transizione `completed\|returned→in_progress`, azzera `completedAt`/`returnedAt`, incrementa `reopenCount`, appende un `correctionEvents` (`type:'reopened'`) nella stessa `writeBatch`. `setSolutionsVisible(true\|false)`: riscrive ogni `questions[i].correctAnswer` (popolato dalle soluzioni congelate, o rimosso — mai lasciato stantio) e `solutionsVisible` in un'unica scrittura su `correctionReturns`. `setReturnVisibleToStudent(bool)`: aggiorna solo `visibleToStudent`, non tocca `corrections`. |
| Scope Security Rules | Lettura/scrittura `corrections`/`correctionEvents` solo owner (mai lo studente, nemmeno in lettura — a differenza di `submissions`, che il proprietario studente legge finché `draft`). Creazione/aggiornamento `corrections` validati contro `submissionId` corrispondente a una `submissions/{id}` esistente con `status == 'submitted'` e `ownerUid` combaciante. Transizioni di stato validate lato Rules con lo stesso set ammesso di `isValidCorrectionStatusTransition`. `correctionEvents`: solo create (mai update/delete), owner-only. `correctionReturns`: scrittura solo owner; lettura owner **e** lo studente proprietario (`studentUid == request.auth.uid`) solo quando il documento esiste **e** `visibleToStudent == true` — non basta l'esistenza del documento. Nessun nuovo indice se il service non esegue ancora query oltre `get()` su path noti. |
| Test minimi | Rules: owner crea/legge/aggiorna `corrections` della propria verifica; owner non può leggere/scrivere corrections di un altro owner; studente non legge mai `corrections`/`correctionEvents`; studente legge `correctionReturns` solo la propria, solo se esiste, e solo se `visibleToStudent == true` (negato quando `false`, anche se il documento esiste); transizione di stato non ammessa rifiutata lato Rules oltre che lato client; `corrections` non creabile per una submission non `submitted` o di un'altra verifica. Service: creazione idempotente (non ricrea se già esiste), salvataggio non tocca campi derivati con valori incoerenti, completamento rifiutato se una domanda è ancora `null` o la mappa è vuota, salvataggio a `reopenCount === 0` non produce mai un evento, salvataggio dopo riapertura produce un evento solo se qualcosa è davvero cambiato (nessun evento su un save che riscrive gli stessi valori), restituzione atomica `corrections`+`correctionReturns`+evento, riapertura produce esattamente un evento e incrementa `reopenCount` di 1, `setSolutionsVisible(false)` rimuove `correctAnswer` da ogni domanda (non lo lascia presente ma "nascosto"). |
...
| Esplicitamente fuori scope M4-01 | UI (M4-02); Registro Correzioni ed export (M4-03); eliminazione submission/correzione; voto elettronico; AI; azioni batch su più correzioni (`visibleToStudent`/`solutionsVisible` restano per-studente in M4-01). |
