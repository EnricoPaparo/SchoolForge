...
| LESSON-MANUAL-02 📐 | Protocollo di valutazione della qualità delle **lezioni generate dal prompt attuale**, prima di modificarlo: dataset JSON con 6 scenari (teoria, procedura, esempi, esercizi, avanzata, confine UDA), rubrica a 15 dimensioni con scala 0–4 e blocker, protocollo di review nelle viste docente/studente e classificazione del problema fra prompt, renderer, metadati e variabilità. | LESSON-MANUAL-01, AIGEN-PROMPT-01, AIGEN-CONTEXT-01 | **Protocollo implementato; verdetto NON DISPONIBILE.** Nessuna generazione, chiamata provider o costo reale; nessun prompt/runtime/dato modificato. Il primo lotto `economy` richiede stima e nuova autorizzazione esplicita. Vedi [evidenze/lesson-manual-02-quality-review.md](evidenze/lesson-manual-02-quality-review.md). |
| LESSON-MANUAL-03 ✅ | Runner locale protetto per eseguire il primo lotto qualitativo congelato: parser dataset e payload fail-closed, profilo solo `economy`, riuso del prompt/schema/validazione/costi AIGEN, dry-run predefinito senza secret, esecuzione reale protetta da due flag + Node 22 + TTY + frase esatta, output/report solo in `functions/lib/` gitignored. | LESSON-MANUAL-02 | **Implementato; solo dry-run eseguito.** 6 chiamate pianificate, ≤12 tentativi, stima 78.698 µUSD (0,078698 USD), tetto 169.910 µUSD (0,169910 USD). Zero chiamate/costo reale; verdetto qualità ancora `NON_DISPONIBILE`; esecuzione reale solo con nuova autorizzazione. |
| LESSON-TUNE-01 ✅ | Benchmark esteso anti-overfitting: i 6 scenari storici restano immutabili e sono combinati con 6 nuovi casi interdisciplinari; split congelato 8 `tuning` + 4 `holdout`, versione del prompt registrata nei report, CLI separata fail-closed e protocollo che vieta di usare gli holdout per ritoccare il candidato già esaminato. | LESSON-MANUAL-03 | **Implementato; solo dry-run eseguito.** Totale 12 chiamate/≤24 tentativi, tetto 349.894 µUSD; tuning 8/≤16, tetto 204.710 µUSD; holdout 4/≤8, tetto 145.184 µUSD. Zero chiamate/costo reale. Protocollo in [evidenze/lesson-tune-01-protocol.md](evidenze/lesson-tune-01-protocol.md). |
