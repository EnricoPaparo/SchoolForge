...
| HARD-01C ⏳ | Region e residenza dati (F02): matrice regioni DEV/PROD (`evidenze/hard-01c-region-matrix.md`) e riconciliazione documentale — corrette le affermazioni assolute «tutto a Milano `europe-west8`» in README/architettura/sicurezza con lo stato reale (DEV Storage/Function `us-central1`; Firestore DEV **`europe-west8` verificata**; target PROD **UE**). **F02 = MITIGATED — contraddizione eliminata; resta il blocker PROD sulla decisione della regione UE per il provisioning ex novo** (`evidenze/hard-01c-human-gate.md`). Audit read-only: nessun servizio creato, nessun dato migrato, nessun deploy. Nessuna modifica a codice/Rules/`firebase.json`/`.firebaserc`/schema/indici/dipendenze. | HARD-00 | **Doc riconciliata e Firestore DEV verificata; resta la decisione regione PROD.** |
| HARD-02A ✅ | Audit accessibilità end-to-end V1 docente+studente (F08): verifica statica + test esistenti dei flussi principali. Verdetto **READY FOR REMEDIATION** — 0 P0, 0 P1, 1 P2 (`DialogShell` senza Escape/focus-trap/restore), P3 di polish. Nessuna modifica a codice. Vedi `evidenze/hard-02a-a11y-audit.md`. | HARD-00 | **Audit svolto; HARD-02A-FIX consigliato.** |
...
| HARD-02B | Implementazione della resilienza import (F06) secondo `hard-02b-import-chunking-design.md`: `publicLessons` import-scoped, chunk ≤400, switch atomico, **cleanup solo delle `publicLessons`** (mai dati tecnici/Storage) con risultato `committed`+`cleanupPending`, +indice `publicLessons(programId,importId)`, **modifica Rules obbligatoria** (non-owner richiede `importId==activeImportId`), campo `LessonDoc.publicLessonId` + helper puro. Ordine rollout indice→Rules→Hosting→smoke. Tocca `importRepository.ts`/`buildImportPayload.ts`/`studentLessonsService.ts`/`repositoryEditorService.ts` + `firestore.rules`/indici. | HARD-02B-00 | Non implementato. |
