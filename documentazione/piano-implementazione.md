...
| VISUAL-ENRICHMENT-05A | **Infrastruttura benchmark locale.** Dataset 8 tuning + 4 holdout con SHA-256, rubriche 6+10, blocker, runner dry-run protetto, provider iniettati, checkpoint/resume e checklist layout/rollout. | VE-04 | **Implementato; tuning reale superato.** Il quarto tuning ha prodotto 8/8 proposte valide, 5 immagini approvate e 2 astensioni appropriate; l'unico stop era pre-provider e a costo zero per una ripetizione esatta di `«Libertà»`. `schoolforge-sketch-prompt/v3` tratta ora l'allowlist come insieme, deduplicando le ripetizioni senza ampliare il vocabolario. La stessa immagine era già stata approvata nel tuning precedente: dopo il merge del nanofix si apre lo split holdout, senza una nuova batteria tuning ridondante. Rollout non eseguito. Evidenza: [visual-enrichment-05a-benchmark.md](evidenze/visual-enrichment-05a-benchmark.md). |
