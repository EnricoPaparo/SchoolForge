...
| VISUAL-ENRICHMENT-05A | **Infrastruttura benchmark locale.** Dataset 8 tuning + 4 holdout con SHA-256, rubriche 6+10, blocker, runner dry-run protetto, provider iniettati, checkpoint/resume e checklist layout/rollout. | VE-04 | **Implementato; tuning superato, primo holdout A non superato.** A ha approvato una immagine e una astensione, registrato un errore transitorio pre-provider e trovato una proposta con 11 etichette che il contratto immagine limitava a 8. Le fonti A sono archiviate e non saranno riutilizzate come validazione. `visual-proposal-01-v4` condivide ora la policy delle etichette con la fase immagine e richiede l'astensione sui contenuti puramente discorsivi; il dataset v2 congela un nuovo holdout B indipendente. Il runner contabilizza `pre_invocation` a costo zero. Holdout B e rollout non eseguiti. Evidenza: [visual-enrichment-05a-benchmark.md](evidenze/visual-enrichment-05a-benchmark.md). |
