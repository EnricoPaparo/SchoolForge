...
| HARD-01B ⏳ | Security header e strategia cache di Firebase Hosting (F03): blocco `headers` in `firebase.json` (X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy, CSP enforced) + cache `no-cache` shell / `immutable` su `/assets/**`; guardrail statico `apps/web/src/hostingHeaders.test.ts`. **F03 = MITIGATED — configurazione pronta, deploy e smoke DEV pending** (`evidenze/hard-01b-dev-smoke.md`); non RESOLVED finché header e login Google non verificati su DEV. Nessuna modifica a Rules/schema/Functions/dipendenze. | HARD-00 | **Configurazione pronta; deploy/smoke DEV pending.** |
| HARD-01C | Region e residenza dati (F02): verifica delle region reali di Firestore/Storage/Functions su PROD e riconciliazione con la documentazione (o correzione della documentazione con l'implicazione di residenza). Decisione del docente; solo documentazione + eventuale re-provisioning Console. | HARD-00 | Non implementato. |
