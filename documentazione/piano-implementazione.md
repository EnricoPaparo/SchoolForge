...
| CONCEPT-MAP-02 ✅ | Persistenza e Rules: `LessonDoc.conceptMapMarkdown`, proiezione `publicLessons` condizionata a `completed === true`, transazione di completamento/scompletamento, audit, test Rules su emulatore. | CONCEPT-MAP-01 | **Implementato.** `saveLessonConceptMap` valida prima della transazione e dimostra identità, owner, import e programma. `setLessonCompleted` è ora la callable server-side autorevole: su pubblicazione valida la mappa privata; su smarcatura conserva il valore privato anche se malformato e rimuove sempre il campo pubblico. Il fingerprint grezzo protegge entrambi i versi dalle race. Limite applicativo 32 KB UTF-8; Rules con limite dichiaratamente più debole in caratteri. Zero costo passivo, listener, polling, chiamate IA o deploy. |
...
| CONCEPT-MAP-04 ✅ | La mappa come **scheda** della lezione: ordine docente Contenuto → Mappa concettuale → Domande → Informazioni, scheda studente data-driven, rimozione dell'azione dal menu «Azioni» e del dialog. | CONCEPT-MAP-03 | **Implementato.** L'assetto precedente era incoerente per modello, non per estetica: il menu «Azioni» contiene operazioni *sulla* lezione, mentre la mappa è un **contenuto** della lezione come il corpo e le domande — e viverne dentro una finestra costringeva il docente a uscire dalla lezione per leggerla. Componente **estratto, non duplicato**: `ConceptMapEditor` porta dentro il pannello la macchina a stati verificata in CONCEPT-MAP-03 (stessa `requestId` fra stima e generazione, validazione autorevole del risultato, conferma prima di rigenerare e prima di abbandonare, guardia anti-doppio-click, nessun aggiornamento dopo unmount); `ConceptMapDialog` rimosso perché restava senza chiamanti. Due modalità perché sono due intenzioni: lettura (la mappa resa come la legge lo studente, o stato vuoto) e modifica (editor, anteprima, salvataggio esplicito). Restano modali solo le due conferme distruttive (`role="alertdialog"`). La mappa entra nella dirty guard **esistente** (`anyDirty`), non in una seconda: cambiare scheda non perde il draft (pannello montato dopo la prima apertura, chiavato sulla lezione), «Annulla» ripristina la baseline, un errore IA non la tocca. Lato studente la tablist **non esiste** senza mappa — nessuna scheda disabilitata, nessun placeholder — e la condizione resta il campo davvero presente nella proiezione, non `completed`: legarla al flag sostituirebbe un confine dati con un confine di rendering. Navigazione da tastiera identica sui due lati (←/→ ciclici, Home, End, roving tabindex, ritorno a «Contenuto» al cambio lezione). Responsive: quattro schede su una riga a 1440/1024 px; a 390/320 la griglia mobile passa da tre a **due** colonne, perché la regola precedente ne fissava tre e la quarta sarebbe finita da sola su una riga. 47 test nuovi o riscritti; suite web 2.956 verdi, Functions 996, Rules 581. Zero nuove letture, query, listener, indici o dipendenze; Functions, prompt, payload, provider, costi, Rules, schema e `saveLessonConceptMap` invariati. Nessun deploy e nessuna chiamata OpenAI reale: rollout DEV e gate umano restano aperti. |
| CONCEPT-MAP-05 ✅ | Quality-only e sintesi ragionata: profilo fisso, Structured Output v2 (`summaryMarkdown` + `diagram`), rimozione dell'ossatura, parser version-aware v1/v2. | CONCEPT-MAP-04 | **Implementato.** *Causa dell'intermittenza:* il selettore Economy/Quality era renderizzato solo nel ramo della modalità modifica, mentre il pulsante di generazione esisteva in entrambe le modalità e leggeva comunque lo stato interno — quindi dalla lettura non compariva alcuna scelta ma un profilo veniva usato lo stesso; e `leaveEdit()` ripristinava bozza, fase, errore e stima ma **non** il profilo, così una scelta Economy fatta in modifica sopravviveva ad «Annulla» restando invisibile. La risposta non è stata replicare il selettore in lettura: `economy` produceva mappe insufficienti, e una mappa sbagliata non è un risparmio ma un ripasso che disinforma. **Doppia difesa:** il client non espone il profilo nella firma di `buildConceptMapRequest` (nessun percorso può produrre Economy), e il server rifiuta comunque fail-closed nella validazione del payload, prima di provider, stima, prenotazione, run e scritture. Pool e lezione conservano entrambi i profili — il vincolo è del kind, non del profilo, e due test espliciti più due hash congelati lo dimostrano. **Artefatto v2:** l'ossatura è rimossa perché sulle generazioni reali si riduceva a un indice e duplicava il ruolo del diagramma; restano sintesi e diagramma, che fanno lavori diversi. Alla sintesi non si chiede più brevità — era la brevità imposta a renderla un sommario inutile — ma copertura dei concetti portanti, relazioni esplicite, termini spiegati e progressione mantenuta, con lunghezza proporzionata al contenuto; resta vietato l'elenco e resta il solo cap tecnico di 32 KB. **Nessuna migrazione:** il parser del documento persistito è version-aware e accetta fail-closed sia v1 sia v2 restituendo il Markdown byte per byte, senza convertire una v1 in v2; la tolleranza è sulla forma, non sui vincoli. Il contratto persistito resta una stringa non vuota entro 32 KB, deliberatamente non irrigidito sulla sola v2. Limite dichiarato: un run Economy legacy non autorizza una nuova generazione Economy (rifiutata alla validazione, prima del replay); i run hanno TTL 24h e l'ordine fail-closed non è stato allentato per recuperarli. `AI_CONCEPT_MAP_PROMPT_VERSION` → `concept-map-05-v3`, quelle di pool e lezione invariate, e resta **non persistita nel run**. UI senza selettore, stima che dichiara «Profilo: Quality», CSS del selettore rimosso perché irraggiungibile. Suite web 2.963 verdi, Functions 1.008; Rules non toccate e non eseguite. Nessuna modifica a Rules, schema, indici, dipendenze, `saveLessonConceptMap`, visibilità o costi passivi. Nessun deploy e nessuna chiamata OpenAI reale. |
| VISUAL-ENRICHMENT-01 ✅ | Proposta testuale e contratti puri: quarto kind IA `visual_proposal`, union discriminata chiusa con l'esito «nessuna immagine utile», validazione del `subject`, tipi e validatori di manifest e ancora, risolutore d'ancora exact-match. | VISUAL-ENRICHMENT-00 | **Implementato.** Fotografia della fase VE-01: allora VE-02→05 restavano aperti; VE-02 è ora implementato separatamente e **Gate GVISUAL resta PENDING**. L'astensione è un esito di **prima classe**: se l'unico esito possibile fosse una proposta, il modello ne produrrebbe sempre una, e un generatore a cui non è consentito astenersi non è un giudizio. Due rami **disgiunti** invece di un booleano con campi opzionali, perché un «nessuna immagine» con la didascalia già scritta non deve essere nemmeno rappresentabile. Payload chiuso a undici chiavi, senza dati studente, URL, riferimenti Storage o hash dichiarati dal client; `modelProfile` letteralmente `quality`, con `economy` rifiutato nella validazione del payload — prima di provider, stima, prenotazione, run e scritture — e verificato sul letterale, non tramite il parser condiviso che accetterebbe `economy` come valido per gli altri kind. Limiti in **code point** e non in unità UTF-16, così un'emoji fuori dal BMP non viene contata doppia; nessun trim, troncamento o normalizzazione. Il `subject` ha un filtro proprio perché sarà l'unico testo variabile a raggiungere il futuro provider di immagini, e i messaggi d'errore non lo replicano mai per intero. Risolutore d'ancora a **solo confronto esatto**: un'illustrazione che riappare sotto un heading somigliante insegna una cosa falsa, una in fondo alla pagina è solo mal impaginata — e non esiste alcun esito «rimuovi». TTL dello staging che **riusa** `AI_CONTENT_RUN_TTL_MS`. Non-regressione verificata: `inputHash` congelati di pool e lezione invariati, forma canonica dei tre kind senza traccia del quarto, prompt/schema/tetti byte-identici, nessuna costante aggiornata per far passare un test. 99 test nuovi; suite Functions 1.221 verdi, suite web 3.496 invariata; zero file `apps/web/**` toccati. |
| CONCEPT-MAP-06 ✅ | Generazione e rigenerazione allineate alla lezione: popup per ogni sessione, scelta esplicita Economy/Quality con Quality predefinito, stima e review prima di applicare la bozza. | CONCEPT-MAP-05 | **Implementato.** Corregge il confine che rendeva intermittente la scelta: l'editor conserva soltanto il draft, mentre profilo, `requestId`, stima, generazione e review vivono in una finestra nuova a ogni pressione. Preview e generate condividono lo stesso payload; cambiare profilo invalida stima e requestId; chiudere e riaprire riparte da Quality. Il backend accetta entrambi i soli profili chiusi. «Usa questa bozza» modifica soltanto il draft locale; `saveLessonConceptMap` resta l'unica scrittura. Artefatto v2, prompt, cap, Rules, schema e visibilità invariati. |
| CONCEPT-MAP-07 ✅ | Tuning della sintesi dopo due generazioni reali: compressione didattica, selezione gerarchica e diagramma non enciclopedico. | CONCEPT-MAP-06 | **Implementato.** Il contratto non chiede più copertura esaustiva né associa complessità a lunghezza: la sintesi deve essere sostanzialmente più breve del corpo tramite selezione dei nuclei e relazioni, senza cap editoriale rigido; massimo un esempio se indispensabile, niente dettagli commerciali o riepilogo ripetitivo salvo centralità esplicita. Diagramma normalmente di 4–7 nodi principali e controllo finale sulle relazioni assolutizzate. `AI_CONCEPT_MAP_PROMPT_VERSION` → `concept-map-07-v1`; payload, parser, cap, persistenza, visibilità e costi invariati. Gate qualitativo reale ancora da eseguire sul candidato. |
| POOL-TUNE-00 ✅ | Infrastruttura del benchmark qualitativo dei pool: 12 fonti reali congelate (8 tuning + 4 holdout), profile probe Economy/Quality, rubrica 10×0–4, blocker e runner locale protetto. | AIGEN-PROMPT-01, LESSON-TUNE-HOLDOUT-01 | **Implementato.** Dataset e sorgenti ancorati a SHA-256; obiettivi di valutazione mai inviati al provider; payload, prompt, validazione, profili, listini e cost model reali riusati senza duplicazione. Dry-run predefinito senza secret/rete; esecuzione reale solo con doppio flag, Node 22, TTY e frase esatta. Zero Firestore/Storage. Il primo profile probe reale del 10/08/2026 è stato interrotto al quinto caso (`PT00-04/economy`, `invocation_unknown`) prima che il vecchio runner scrivesse output: nessun verdetto e fase ancora aperta. `POOL-TUNE-01` profile probe, `02` tuning, `03` holdout e Gate GPOOL-QUALITY restano aperti. Vedi `pool-quality-roadmap.md`. |
| POOL-TUNE-01-RECOVERY ✅ | Checkpoint atomico e ripresa fail-closed dei benchmark pool reali. | POOL-TUNE-00 | **Implementato nel runner locale.** Directory creata prima della rete, report `running/failed/complete`, salvataggio dopo ogni risposta revisionabile, proposta valida ricaricata attraverso il contratto semantico e confronto col DTO canonico, coerenza di dataset/rubrica/prompt/piano/prefisso/usage/listino/costi/errore. `--resume-session` richiama soltanto il suffisso mancante e richiede una nuova conferma; un checkpoint pieno ma non finalizzato si chiude senza API key né provider. Output confinati a `functions/lib`; zero Firestore/Storage/callable e nessuna chiamata OpenAI nei test. Il tentativo del 10/08 precedente alla modifica non è recuperabile perché non aveva scritto alcun checkpoint. |
| POOL-TUNE-01-EVIDENCE ✅ | Conservazione degli output semanticamente invalidi e recovery del checkpoint v1. | POOL-TUNE-01-RECOVERY | **Implementato nel runner locale.** Un output ricevuto ma rifiutato dal validator non interrompe più il lotto: raw output, errore riproducibile, usage e costo vengono persistiti separatamente nel report v2 e la combinazione conta come già eseguita; gli errori provider senza risposta restano invece ritentabili. Il checkpoint reale dell'11/08 su `PT00-01/economy` è migrato fail-closed come `legacy_checkpoint_without_raw`, senza inventare risposta, usage o costo: il blocker «soluzione fuori dalle opzioni» è conservato e restano 7 chiamate. Testata persistenza, manomissione, continuità del lotto, migrazione v1 e distinzione dall'errore provider. Nessun deploy o chiamata reale aggiuntiva. |
| POOL-TUNE-01 ✅ | Profile probe reale Economy/Quality e scelta del profilo per il tuning. | POOL-TUNE-01-EVIDENCE | **Completato e revisionato.** Economy fallisce 4/4 scenari: un output invalido e tre pool con chiavi di risposta spostate come indici 1-based. Quality ha 4/4 output validi, 17/17 soluzioni chiuse e 13/13 aperte corrette; due PASS e due revisioni locali per duplicazione e `\\n` letterali. Quality selezionato per `POOL-TUNE-02`, senza promozione runtime. Costo noto Quality 68.259 µUSD; totale Economy non ricostruibile per il checkpoint legacy. Evidenza: `evidenze/pool-tune-01-profile-review.md`. `POOL-TUNE-02`, holdout e Gate GPOOL-QUALITY restano aperti. |
...
**Gate GVDIF PASS** il 15 agosto 2026 su `schoolforge-dev`; evidenza in
...
| Pacchetto | Sintesi | Dipendenze | Stato |
|---|---|---|---|
| VDIF-00 ✅ | Contratto tecnico definitivo (modello dati congelato, perimetro privacy a elenco chiuso, mutua esclusione VEX bidirezionale, guardie di attivazione G01→G21 con ordine di letture/scritture e confine transazionale, snapshot e `assignedQuestionOrders`), cost model, matrice minima dei test, roadmap eseguibile VDIF-01→05 + GVDIF + ESITI-01, e **prototipo UI statico** responsive. | GVEX PASS, M4, `resolveAssignedQuestions` | Fase documentale completata; il percorso successivo è ora interamente distribuito su DEV e Gate GVDIF PASS. |
| VDIF-01 ✅ | **Registro etichette owner-only**: `DifferentiationLabelDoc` a **otto chiavi** (`labelId`, `ownerUid`, `name`, `nameKey`, `assignedCount`, `draftUsageCount`, `createdAt`, `updatedAt`), prenotazione `differentiationLabelNames/{SHA-256(ownerUid + U+0000 + nameKey)}` a quattro chiavi, helper puri `labelName.ts` (trim, collasso spazi Unicode, limiti 40 code point / 120 byte, no caratteri di controllo) e `labelReservationId.ts` (Web Crypto, fail-closed), service canonico con **CRUD interamente transazionale** e audit nello stesso commit, Rules owner-only a contratto chiuso su entrambe le collezioni, **terza scheda «Etichette»**. | VDIF-00 | **Implementato e distribuito su DEV.** Nessun indice nuovo, nessuna lettura per card, listener, polling o Function. Gate GVDIF PASS. |
| VDIF-02 ✅ | **Assegnazione privata studente → etichetta**: `studentLabelAssignments/{studentUid}` owner-only a cinque chiavi, contatori transazionali, audit e controllo «Etichetta» nella card studente. | VDIF-01 | **Implementato e distribuito su DEV.** Una query per caricamento, zero letture per card e zero refetch dopo una mutazione; privacy verificata strutturalmente. Gate GVDIF PASS. |
| VDIF-03 ✅ | **Builder delle varianti** con anteprima reale, mutua esclusione VEX bidirezionale e salvataggio transazionale dei contatori `draftUsageCount`. | VDIF-02 | **Implementato e distribuito su DEV.** Zero operazioni sulle etichette quando l'insieme non cambia. Gate GVDIF PASS. |
...
| VISUAL-ENRICHMENT-01 | **Proposta testuale e contratti.** `kind: 'visual_proposal'` di prima classe in `AiContentRequest`; payload chiuso con profilo forzato a `quality`; prompt dedicato con versione propria e corpo lezione delimitato come dato non attendibile; Structured Output strict con l'esito «nessuna immagine utile» di prima classe; validatore del `subject`; tipi e validatore puro del manifest; risolutore d'ancora puro; test di **non-regressione byte-identica** degli `inputHash` di pool, lezione e mappa. Backend-only e puro. | VE-00 | **Implementato.** |
| VISUAL-ENRICHMENT-02 | **Catena binaria completa.** Provider Image API iniettato; callable preview/generate; normalizzazione server-side — magic bytes, dimensioni reali, resize a lato lungo 1200 px, WebP, rimozione metadati, cap rigido 200 KB, `sha256` sui byte finali; `visualRuns` closed-world, staging con TTL 24 h, replay senza seconda spesa e cleanup delete-trigger; cost model ufficiale. | VE-01 | **Implementato, non distribuito.** Default disabled; zero chiamate OpenAI reali. |
| VISUAL-ENRICHMENT-03 | **Persistenza, proiezione e lifecycle — chiuso.** VE-03A (ticket, manifest, promozione), VE-03B (completamento server-side, rimozione, abbandono, cleanup delle cancellazioni) e VE-03C (export ZIP binario, integrazione end-to-end, cost model misurato) sono implementati e **non distribuiti**. L'export aggiunge `visuals/{assetId}.json` e `.webp` tramite l'unica callable binaria, owner-only e senza percorsi dal client; l'import continua a ignorare del tutto i file visuali. | VE-02 | **Chiuso.** Nessuna UI, nessun deploy. |
| VISUAL-ENRICHMENT-04 | **UI e renderer — chiuso nel codice.** VE-04A: renderer, letture condizionali e riancoraggio. VE-04B: dialog docente a macchina di stati esplicita, proposta testuale e immagine con costi/conferme distinti, bind/generate/promote, sostituzione, abandon/remove e refresh autorevole. | VE-03 | **Chiuso, non distribuito.** GVISUAL PENDING. |
| VISUAL-ENRICHMENT-05A | **Infrastruttura benchmark locale.** Dataset 8 tuning + 4 holdout con SHA-256, rubriche 6+10, blocker, runner dry-run protetto, provider iniettati, checkpoint/resume e checklist layout/rollout. | VE-04 | **Implementato; tuning superato, holdout A, B e C non superati.** C ha confermato tre decisioni corrette ma un confronto ordinato non dimostrato, su una sorgente essa stessa fisicamente ambigua. Tutti sono archiviati e non saranno riutilizzati come validazione. `visual-proposal-01-v6` e `schoolforge-sketch-prompt/v5` impongono origine/scala comune ed estremi distinti; il dataset v4 congela un holdout D indipendente. Holdout D e rollout non eseguiti. Evidenza: [visual-enrichment-05a-benchmark.md](evidenze/visual-enrichment-05a-benchmark.md). |
| VISUAL-ENRICHMENT-05 | **Benchmark qualitativo reale e rollout DEV.** Review umana, tasso di «nessuna immagine utile», peso/tempi/layout shift reali e rollout controllato. | VE-05A | **Aperto.** |
...
| Pacchetto | Sintesi | Dipendenze | Stato |
|---|---|---|---|
| AIGEN-COMPLETE-01 | Aggiunge «Generazione completa IA» nelle Azioni lezione. Dopo preview valida, pulisce automaticamente contenuto/mappa/pool/immagini preservando i metadati e orchestra in Quality `contenuto → mappa → pool configurato → immagini auto 0–3 → refresh`. Progresso per fase, riepilogo costi e retry della sola fase incompleta. | AIGEN-03, CONCEPT-MAP, pool IA, MULTI-VISUAL-04 | **Implementato su branch di sviluppo.** Modifiche web, test e documentazione; nessuna nuova Function, Rule, collection, configurazione, dipendenza o provider. |
| Gate GCOMPLETE | Smoke DEV su sostituzione di una lezione piena, default 5/3/2 e valori personalizzati, mappa, 0/1/3 immagini, errore/retry di una fase, budget insufficiente e responsive mobile. Verifica metadati invariati e assenza di nuova spesa per fasi già concluse. | AIGEN-COMPLETE-01 | **PENDING.** |
