...
| VISUAL-ENRICHMENT-05A | **Infrastruttura benchmark locale.** Dataset 8 tuning + 4 holdout con SHA-256, rubriche 6+10, blocker, runner dry-run protetto, provider iniettati, checkpoint/resume e checklist layout/rollout. | VE-04 | **Implementato; tuning superato, holdout A e B non superati.** A ha trovato una divergenza fra proposta e contratto immagine; B ha prodotto decisioni corrette ma un confronto spaziale non distinguibile rispetto al riferimento comune. Entrambi sono archiviati e non saranno riutilizzati come validazione. `visual-proposal-01-v5` e `schoolforge-sketch-prompt/v4` rendono eseguibili i confronti fra posizioni/stati; il dataset v3 congela un holdout C indipendente. Holdout C e rollout non eseguiti. Evidenza: [visual-enrichment-05a-benchmark.md](evidenze/visual-enrichment-05a-benchmark.md). |
