...
| SGW-01 🚧 | Function `repositoryGateway` + client adapter; read/write/delete singolo; migrazione contenuto/metadata lezioni e UDA e pool (load/save/delete); test sicurezza; deploy DEV + smoke Brave. | SGW-00 | **Codice implementato e testato** (gateway core + adapter + migrazione righe 1–7); rewrite `/api/repository/**` in `firebase.json`; dipendenze `firebase-functions`/`firebase-admin` aggiunte. **Deploy DEV e smoke Brave ANCORA da eseguire** (richiede Docente; region da confermare). |
