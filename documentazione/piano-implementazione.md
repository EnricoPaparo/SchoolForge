...
| M5-05D1 ✅ | Guardrail server-side prima del provider reale: config runtime `settings/aiConfig` fail-closed (kill switch senza deploy, letta solo server-side, mai lato client), limiti prudenziali DEV nel preflight; listino prezzi versionato e ledger budget come moduli puri testati non ancora collegati. | M5-05C | **Implementato ma provider reale non attivabile.** Mock e sole-chiuse invariati a 0 costo; nessuna chiave/chiamata/costo/deploy; budget-transaction, retry backoff, privacy/`expireAt` di `aiCorrectionRuns` e Rules restano a M5-05D; Human Gate/G7 aperti. |
