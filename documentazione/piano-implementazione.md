...
| VISUAL-ENRICHMENT-04 | **UI e renderer — suddiviso in A + B.** **VE-04A implementato, non distribuito**: renderer con split dei token e doppia sanificazione, `<figure>` React controllata, lettura dei byte solo in presenza di manifest, fallback e riancoraggio server-side senza rigenerazione, vista studente condizionale. **VE-04B aperto**: dialog a dieci stati, pulsante «Arricchisci visivamente» e workflow di generazione. | VE-03 | **Aperto** fino a VE-04B. |
