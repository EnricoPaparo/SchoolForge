...
**Versione:** 1.0
**Data:** 10 luglio 2026
**Stato:** ✅ completato — M3F-00→M3F-11C completati, rollout M3F-08 eseguito in DEV, Gate G5 superato con evidenze automatiche e smoke manuale DEV. Il flusso include sessione obbligatoria, Modalità verifica per le classi coinvolte, proiezione sicura `publicLessons.content`, controllo `studentPdfEnabled`, monitor consegne ed autosave dirty-only/concorrenza consolidati. Vedi `documentazione/evidenze/g5-m3-full-checklist-finale.md` per la checklist finale del gate.
...
| **M3F-11** ✅ | Integrazione, migrazione DEV e gate finale | checklist/smoke M3F, deploy Rules+indici+Hosting | M3F-06→M3F-10 | Migrazione DEV verificata; smoke docente confermato manualmente su DEV; sicurezza diretta testata; Gate G5 superata — vedi `documentazione/evidenze/g5-m3-full-checklist-finale.md`. |
...
| **PERF-SEC-01A** ✅ | Audit evidence-based prestazioni, costi Firebase e sicurezza | `documentazione/performance-security-audit.md` (nuovo) | M3F-11C | Mappa accessi Firebase per flusso; stime scenari A/B/C; review statica Rules senza gap enforced trovati; finding classificati P0-P3 con evidenza file/riga; nessuna modifica a codice applicativo o Rules. |
| **PERF-SEC-01B** ✅ | Remediation dei soli finding approvati dell'audit PERF-SEC-01A | dipende dai finding approvati (vedi `performance-security-audit.md` §9) | PERF-SEC-01A | Solo i finding esplicitamente approvati vengono implementati; nessuna modifica non richiesta dall'audit; verifiche mirate per ogni finding remediato secondo la colonna "Verifica necessaria" del report. |

**PERF-SEC-01A — audit prestazioni/costi/sicurezza:** analisi statica evidence-based di query/listener/write Firestore, Storage, Security Rules e bundle frontend sull'intero prodotto fino a M3F-11C. Nessuna ottimizzazione applicata in questa fase — solo diagnosi, misure (bundle reale da `pnpm build`) e priorità. Vedi `documentazione/performance-security-audit.md`.

**PERF-SEC-01B — remediation mirata (completa: 01B-1 → 01B-4):** implementa i finding dell'audit PERF-SEC-01A esplicitamente approvati, in quattro pacchetti coerenti. 01B-1: atomicità `setVerificationVisibility`/`closeVerification` (PERF-05). 01B-2: latenza `savePool`/`loadSelectedQuestionsWithSolutions` (PERF-04, PERF-09). 01B-3: letture mirate `countPendingStudents`/guard di cancellazione (PERF-08, PERF-02 parziale); PERF-01 (storico verifiche) esplicitamente rimandato con soglia di rivalutazione documentata, non implementato. 01B-4: `React.lazy` di `TeacherShell`/`StudentShell` al confine docente/studente in `App.tsx`/`RoleGate.tsx` (PERF-10) — entry iniziale da 1 194.56 KB a 647.00 KB minificati (-45.8%), da 323.04 KB a 164.65 KB gzip (-49.0%), nessun micro-chunking per vista; pulizia dell'unico import dinamico isolato di `firebase/firestore` in `submissionsService.ts` (PERF-06), warning Vite eliminato. Non introduce ottimizzazioni non derivate dall'audit.

**Gate prestazioni/sicurezza (PERF-SEC-01) — prerequisito per la chiusura definitiva di M3-full:** prima di dichiarare M3-full definitivamente chiuso (oltre al Gate G5 già definito sotto), i finding P0/P1 dell'audit PERF-SEC-01A devono essere risolti o esplicitamente derogati con motivazione scritta; i finding P2/P3 possono essere rimandati a un ciclo successivo se documentato.
...
2. **Esecuzione del backfill** (owner-only, `publicLessonsBackfillService.backfillPublicLessonsContent`, trigger nell'avviso manutenzione di `DidatticaView`) e verifica dei conteggi restituiti (`analyzed`/`migrated`/`skipped`/`failed`) — rilanciabile finché `failed` non è vuoto.
...
**Gate G5 — verdetto: superato.** Checklist finale con evidenze automatiche e conferme manuali DEV per ciascun criterio in `documentazione/evidenze/g5-m3-full-checklist-finale.md`.
