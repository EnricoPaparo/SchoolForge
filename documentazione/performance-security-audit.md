...
**SEC-02 — `teacherSnapshot` non conteneva una copia immutabile delle domande, contraddicendo il requisito di snapshot immutabile — ✅ RISOLTO (fix indipendente da PERF-SEC-01B)**
- Evidenza: `teacherSnapshot` (prima del fix) conservava solo `questionRefs` — puntatori stabili ai pool *correnti* in Storage — mai il testo/opzioni/soluzione delle domande al momento dell'attivazione.
- Impatto: modificare o eliminare un pool dopo l'attivazione di una verifica poteva silenziosamente cambiare il contenuto del PDF docente (normale e con soluzioni) di una verifica già `active`/`closed`, o rendere impossibile generarlo — non un problema di confidenzialità (i dati restavano owner-only), ma di **integrità/immutabilità** dello snapshot, in contraddizione con l'invariante dichiarato altrove nel prodotto ("l'attivazione congela le domande").
- Scenario che lo attiva: qualunque modifica o cancellazione di un pool dopo l'attivazione di una verifica che referenzia le sue domande.
- Soluzione implementata: `teacherSnapshot.questions?: VerificationTeacherQuestionSnapshot[]` — copia completa (testo, opzioni, soluzione, maxPoints, order) scritta una sola volta all'attivazione, nella stessa transazione già esistente. I PDF di una verifica `active`/`closed` con `questions` presente sono ora costruiti direttamente dallo snapshot, **zero letture Storage**, nessuna dipendenza dai pool correnti. Le verifiche legacy (attivate prima del fix, senza `questions`) continuano temporaneamente a rileggere i pool correnti — comportamento invariato per loro, nessuna migrazione automatica.
- Beneficio atteso: correttezza (PDF davvero immutabile per le nuove attivazioni) + un effetto collaterale di costo: 0 letture Storage per il download PDF di una verifica `active`/`closed` con snapshot completo, contro 1+ letture Storage per pool distinto prima del fix (e per ogni verifica legacy tuttora).
- Rischio della modifica: basso — cambio additivo (`questions?` opzionale), fallback legacy esplicito e testato, nessuna Rule aggiuntiva necessaria (il campo eredita l'immutabilità già garantita dalle regole di update esistenti su `verifications/{docId}`).
- File coinvolti: `types/firestore.ts`, `verificationsService.ts`, `verificationPdf.ts`, `verificationSnapshotMappers.ts` (nuovo), `verificationSnapshotLimits.ts` (nuovo), `VerificationsView.tsx`.
- Verifica necessaria: test service (`activateVerification` scrive `questions`, ordine, pool letto una sola volta, blocco pre-transazione su pool/soluzione invalidi e su snapshot troppo grande) + test vista (PDF da snapshot senza Storage, fallback legacy invariato) — implementati.
...
## 5. Nota SGW — costi/prestazioni del gateway (SGW-01 operativo; SGW-02A nel codice)

Il **Repository Storage Gateway** (contratto in [storage-gateway-roadmap.md](storage-gateway-roadmap.md)) è operativo su DEV per SGW-01. SGW-02A aggiunge nel codice la cancellazione server-side di una root import e riduce i round-trip Firestore di create/save; import/export/backfill e letture batch restano da migrare. Impatto costi/prestazioni:
...
- **Una chiamata per operazione logica**: SGW-02A realizza questo principio per la cancellazione di ciascun import. Import/export/verifiche diventeranno batch nel resto di SGW-02; non sono ancora dichiarati migrati.
...
- **Metriche/soglie**: invocazioni/giorno, errori 4xx/5xx, latenza p50/p95, cold start, operazioni Storage A/B, egress GB; superamento del budget alert o egress in crescita non spiegata → rivalutare l'architettura. SGW-02A misura per ora la riduzione strutturale dei round-trip; non sostituisce misure DEV reali.
