...
| VEX-02A ✅ | Svolgimento studente della variante assegnata. | Routing fail-closed su `distributionMode`; avvio/ripresa/refresh via callable idempotente; `OnlineExamView` solo variante; autosave/consegna ristretti (client + Rules `answers ⊆ assignedAnswerKeys`); PDF studente personale via callable senza avvio della submission; `same_questions` invariato (nessuna callable). |
...
| VEX-02C ✅ | Builder assistito locale dei gruppi. | Precompilazione tecnica UDA+tipo+difficoltà (`vexAutogroup.ts`, fail-closed) + abbinamento progressivo; **modifiche manuali autorevoli** (nessun ricalcolo globale); avviso permanente «tecnica, non pedagogica»; selettore leggibile (listbox accessibile a due righe con `questionPreview`). Puramente client, **zero costo/backend**; `same_questions` invariato. |
| VEX-03A ✅ rollout DEV / Gate GVEX ✅ PASS | Rollout coordinato Functions → Firestore Rules → Hosting su `schoolforge-dev`: baseline server/Rules SHA `1399faeb1539b1adf1fd9d0ead1bb485ca5d9d53`; Hosting finale VEX-02C SHA `adba8e3208c33ece05fbc928f598e0197c4ba94b`. | Preflight automatico verde e smoke manuale docente/multi-studente confermato il 21 luglio 2026; nessuna fuga di alternative/soluzioni. Evidenze in `evidenze/gvex-human-gate.md`. |
