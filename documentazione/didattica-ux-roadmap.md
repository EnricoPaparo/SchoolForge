...
| UI-STUDENTI-CLASSI-01 ✅ | Sezione docente «Studenti / Classi» allineata al linguaggio di Didattica e Verifiche: tabelle sostituite da liste verticali di record card full-width, azioni discrete nel menu «…» condiviso, «Nuova classe» come azione a larghezza piena su `DialogShell`. La classe dello studente resta modificabile **dalla card**, con label visibile e select full-width su mobile. **Implementato.** | UI-VERIFICHE-06B | Tutte le funzionalità precedenti conservate (filtri, ricerca, ordinamento, stati, conferme, guardie, autorizzazioni); aggiunto «Primo accesso», già presente nei dati caricati. Nessuna Rule, indice, schema, query, listener o dipendenza modificata: stesse letture e stesse scritture di prima. |
| UI-CONSEGNE-01 ✅ | Schermata consegne responsive: la tabella resta su desktop (righe più compatte, `Aggiornato alle HH:mm:ss` accanto al titolo) e sotto `44rem` lascia il posto a card full-width con Punteggio/Visibilità affiancati, Stato a tutta riga, data e menu «…» di riga. Toolbar massiva a sette comandi su desktop, «Correggi con IA» + menu «Azioni selezionate» su mobile. «Torna alle verifiche» diventa il controllo ghost «← Verifiche». **Implementato.** | UI-STUDENTI-CLASSI-01 | Una sola rappresentazione montata alla volta: nessun duplicato per gli screen reader. Selezione, ordinamento, filtri, refresh, apertura correzione, eventi e azioni batch invariati; desktop e mobile invocano le stesse funzioni. Zero nuove letture, scritture, query o listener Firestore. |
| FORCE-SUBMIT-01 ✅ | Chiusura e consegna forzata di una verifica **iniziata e non consegnata**: nella schermata consegne il docente dispone dell'azione «Chiudi e consegna» (icona di riga su desktop, voce del menu «…» su mobile, stessa derivazione enabled/disabled) con conferma esplicita che dichiara l'irrecuperabilità del testo non ancora salvato. La transizione `draft → submitted` avviene **server-side** nella callable transazionale `forceSubmitSubmission`; la ricevuta e la schermata di conferma dello studente distinguono «Consegna acquisita dal docente» da una consegna ordinaria. **Implementato.** | UI-CONSEGNE-01 | Nessuna consegna viene mai creata per chi non ha iniziato; `lastSavedAt` non viene mai riscritto; `forcedByTeacher` è server-only e vale esattamente `true`. Rules **non modificate**: i key-set chiusi già esistenti negano il campo al client (dimostrato da test emulator dedicati). **Costo:** zero costo passivo, nessun listener/polling, nessun indice; una chiusura effettiva = 1 callable + 3 letture puntuali in transazione + 2 scritture atomiche; un replay = 0 scritture. |
| FORCE-SUBMIT-02 ✅ | La chiusura forzata diventa **azione massiva con preavviso**: sparisce dalle singole righe e card e compare come «Chiudi consegne» subito a destra di «Azzera» (ultima voce del menu batch su mobile, nessun ottavo pulsante visibile). La conferma dichiara i 60 secondi, quante consegne verranno programmate e quante selezioni sono escluse, con il motivo. Lo studente vede un banner fisso non chiudibile con countdown, «Salva ora» e ultimo salvataggio; fino alla scadenza continua a compilare, salvare e consegnare. Corretti anche il trattino duplicato nella colonna Azioni e la larghezza della colonna Eventi. **Implementato.** | FORCE-SUBMIT-01 | Selezionabile ogni riga con una consegna reale, bozze incluse; «Non iniziata» resta non selezionabile e nessuna consegna viene mai creata. Le azioni di correzione mantengono il proprio perimetro. Countdown ricalcolato dalla scadenza server-side, `role="alert"` solo alla comparsa. Un solo listener sul proprio documento, nessun polling, Rules **non modificate**, nessun indice nuovo. |
| UI-EXAM-SUBMISSIONS-01 ✅ | Chiusura UI consegne/esame: intestazione mobile ordinata `Verifiche → Consegne online → titolo → metadati → export`, solo badge di stato, card consegna inerte con eventi e correzione su controlli espliciti; navigatore domande visualmente condiviso fra svolgimento, correzione docente e revisione studente, con anello corrente non tagliato e hover arancione su puntatore fine; intestazione esame con identità studente e accento SchoolForge; conferma consegna senza classe. **Implementato.** | UI-CONSEGNE-01 | Solo componenti/CSS/test web. Dati, servizi, query, listener, Rules, Functions, indici, schema, Storage e costi invariati. |
...
| UI-VERIFICHE-MOBILE-01 ✅ | Compattazione della card docente esclusivamente mobile: rimossa la fascia vuota della CTA non visibile su touch; il trigger «Azioni» è icon-only (`…`), con target 44 px in alto a destra. La riga metadati mostra inoltre un indicatore documento solo quando il PDF studente è abilitato, su desktop e mobile. **Implementato.** | UI-RECORD-ACTIONS-01 | Nessun dato o costo nuovo: riuso di `studentPdfEnabled` già caricato. Desktop invariato e smoke Chromium responsive. |
| UI-DIDATTICA-ACTIONS-01 ✅ | Allineamento del trigger «Azioni» delle card corso all'angolo superiore destro anche su desktop, come nelle card verifica; eliminato il precedente centraggio verticale. **Implementato.** | UI-RECORD-ACTIONS-01 | Solo presentazione; menu, handler, mobile e dati invariati. |
| STRUCTURE-IMPORT-00 | Contratto dell'importazione strutturale di UDA e lezioni senza contenuto. Nuove voci esclusivamente nei menu esistenti: `Azioni corso → Importa struttura UDA` e `Azioni UDA → Importa lezioni`. | DUX-04D, TWU-04B, AIGEN-03 | **Progettato, non implementato** — vedi `structure-metadata-import-roadmap.md`. Nessun nuovo pulsante permanente, dato o costo passivo. |
| STRUCTURE-IMPORT-01→03 ✅ | Parser/planner, append UDA, append lezioni e contesto IA UDA bounded. | STRUCTURE-IMPORT-00 | **Implementati e validati su DEV. Gate GSTRUCT PASS** — vedi `evidenze/gstruct-checklist-finale.md`. |
