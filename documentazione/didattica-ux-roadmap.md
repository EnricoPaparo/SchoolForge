...
| DUX-04B | Editing completo **lezione** nel workspace: editor contenuto Markdown, modifica metadata lezione, creazione/eliminazione lezione (con guard verifiche). | DUX-04A | Riuso dei service Repository Editor esistenti; dirty-guard esteso all'editor contenuto; nessuna nuova Rule. |
| DUX-04C | Navigazione mobile a livelli (drill-down definitivo), pulsante Indietro coerente, modalità **Organizza** (riordino UDA/lezioni con frecce). | DUX-04B | Parità di riordino con `LessonsView`; nessun bottone annidato; nessuna nuova Rule. |
