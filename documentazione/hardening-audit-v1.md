...
- **Stato (dopo HARD-01C):** **MITIGATED — contraddizione documentale eliminata; blocker PROD + verifica Firestore DEV aperti.** Le affermazioni assolute «tutto a Milano `europe-west8`» in `README.md`/`architettura.md`/`sicurezza.md` sono state corrette con lo stato reale (DEV Storage/Function `us-central1` verificato; Firestore DEV **NON VERIFICATA**) e il target PROD **UE**; matrice in `evidenze/hard-01c-region-matrix.md`. **Non RESOLVED**: restano la verifica della regione Firestore DEV (nessuna CLI in sessione) e la decisione di regione UE per il provisioning PROD ex novo — vedi `evidenze/hard-01c-human-gate.md`. Nessun servizio creato, nessun dato migrato.
...
- **File probabilmente coinvolti:** `architettura.md`, `README.md`; eventuale re-provisioning Console (fuori repo). **Test:** n/a. **Confidenza:** media (region bucket/Function da commento-codice con output gcloud citato; region Firestore non verificata live in questa sessione).
...
- **HARD-01 — Operatività & configurazione (P2).** Suddiviso in tre sotto-pacchetti indipendenti:
  ...
  - **HARD-01C** (**MITIGATED — blocker PROD + verifica Firestore DEV**) — **F02**: matrice regioni e riconciliazione documentale (contraddizione «tutto a Milano» corretta) in `evidenze/hard-01c-region-matrix.md`; restano la verifica della regione Firestore DEV e la decisione di regione UE per il provisioning PROD ex novo (`evidenze/hard-01c-human-gate.md`).
