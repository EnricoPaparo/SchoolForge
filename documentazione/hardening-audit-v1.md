...
- **Stato (dopo HARD-02B-1):** **fondazione implementata, non ancora RESOLVED.** HARD-02B-1 ha introdotto gli ID `publicLessons` import-scoped (`LessonDoc.publicLessonId` + helper `publicLessonId.ts`), la query studente `programId + activeImportId`, le **Rules obbligatorie** `importId == activeImportId` (non-owner) e l'indice `publicLessons(programId, importId)`; la transazione import resta atomica. Resta **HARD-02B-2** (chunking ≤400, macchina a stati staging/active/superseded, switch atomico, cleanup differito) per chiudere F06. Progettazione:
