...
| `scheduleForceCloseSubmissions` + `runScheduledForceClose` (**FORCE-SUBMIT-02 implementato**) | Verifiche online | Chiusura **multipla** con preavviso fisso di **60 secondi**. Sostituisce la callable per singola consegna di FORCE-SUBMIT-01, **rimossa** (avrebbe permesso di chiudere senza il preavviso promesso allo studente); il suo core transazionale `forceSubmitCore.ts` resta ed è riusato. **`scheduleForceCloseSubmissions`** — `onCall` v2 `us-central1`, scale-to-zero. Input **chiuso** `{ verificationId, studentUids[] }`: uid unici, array non vuoto, cap **60**, id validati come document ID Firestore sui **byte UTF-8** (incluso l'id concatenato) prima di costruire qualunque `DocumentReference`; ogni altra chiave (`ownerUid`, `submissionId`, `requestId`, `deadline`, `graceSeconds`, `status`, `forcedByTeacher`) ⇒ `invalid-argument`. Autorizzazione owner fail-closed sulla verifica; poi, **per studente**, una transazione puntuale che programma **solo** le bozze non già programmate: **una** scrittura di tre marcatori server-only (`forceCloseRequestId`, `forceCloseDeadline`, `forceCloseRequestedAt`) e **una** Cloud Task con `scheduleTime` a +60 s. **Nessuna consegna viene creata** per chi non ha iniziato (`not_started`, zero scritture/task). Programmare **non consegna nulla**: stato, risposte, flag, eventi e `lastSavedAt` restano intatti e lo studente può ancora salvare e consegnare. Un doppio click o un retry ritrovano `already_scheduled`: **nessuna seconda task**. Se l'accodamento fallisce il marcatore viene rimosso (compensazione), così non resta un banner senza chiusura. Un fallimento individuale non interrompe le altre righe (`failed`). Risposta sanitizzata `{ graceSeconds, results: [{ studentUid, outcome }] }` — nessun contenuto, codice, `requestId` o scadenza. **`runScheduledForceClose`** — task queue (`onTaskDispatched`, `maxAttempts: 5`, `maxConcurrentDispatches: 10`): alla scadenza rilegge in **una** transazione verifica + submission + ricevuta, valida il payload in modo chiuso e agisce **solo** se ritrova esattamente la propria richiesta; poi riusa `decideForceSubmit`/`forceSubmitWrites` per le stesse **due** scritture atomiche di FORCE-SUBMIT-01, rimuovendo i tre marcatori nello stesso update (nessuna terza scrittura). Consegna normale nel frattempo, riprogrammazione, programmazione rimossa, consegna eliminata, verifica passata ad altri, retry o consegna doppia della task ⇒ **no-op sicuro, zero scritture**. Nessuna Function resta in attesa 60 secondi; nessun timer del browser partecipa alla decisione. Nessun nuovo indice, nessuna dipendenza, nessun polling. |
