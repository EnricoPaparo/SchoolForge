...
| `forceSubmitSubmission` (**FORCE-SUBMIT-01 implementato**) | Verifiche online | `onCall` v2, region `us-central1`, scale-to-zero (`minInstances: 0`, `maxInstances: 3`). Il **docente** acquisisce e chiude una verifica online che lo studente ha iniziato ma non ha consegnato. Input **chiuso** `{ verificationId, studentUid }`: qualunque altra chiave (`ownerUid`, `submissionId`, `answers`, `deliveryCode`, `status`, `submittedAt`, `lastSavedAt`, `forcedByTeacher`) ⇒ `invalid-argument`; gli id devono essere segmenti Firestore validi. Autorizzazione fail-closed: auth ⇒ verifica esistente ⇒ `ownerUid == auth.uid` ⇒ submission esistente ⇒ coerenza (`submissionId`/`verificationId`/`studentUid`/`ownerUid`). **Non crea mai una consegna** per uno studente che non ha iniziato (`not-found`). Transazione unica: legge verifica + submission + ricevuta, poi **due sole scritture** nello stesso commit — `submissions/{id}` (`status:'submitted'`, `deliveryCode`, `submittedAt`, `forcedByTeacher:true`) e `submissionReceipts/{id}` (dati canonici presi dalla submission + `forcedByTeacher:true`). **`lastSavedAt` non viene mai toccato**: continua a rappresentare l'ultimo salvataggio reale dello studente. `answers`/`flagged`/`attentionEvents`/`assignedQuestionOrders`/`assignedAnswerKeys`/`startedAt` restano invariati: si congela l'ultima versione **già salvata**, il testo mai autosalvato è irrecuperabile. Codice consegna generato server-side nel formato canonico `SF-YYYY-XXXX` (RNG `node:crypto`). Idempotenza: un replay su una chiusura già completata ⇒ `{ status: 'submitted' }` con **0 scritture**; una consegna normale avvenuta nel frattempo ⇒ `{ status: 'already_submitted' }` e **non viene mai sovrascritta**. Entrambi gli esiti richiedono una ricevuta esistente e **completamente coerente** con la submission (`submissionId`, `verificationId`, `studentUid`, `ownerUid`, `deliveryCode`, `submittedAt` confrontato in modo deterministico, `verificationTitle`, `className`) e un marcatore coerente — `forcedByTeacher === true` su entrambi i documenti per la chiusura forzata, **completamente assente** su entrambi per la consegna normale; `false`/`null`/altri valori sono uno stato incoerente. Ricevuta assente, malformata o divergente in un solo campo ⇒ `failed-precondition`, **0 scritture**. Una submission ancora `draft` non può avere alcuna ricevuta: se esiste ⇒ `failed-precondition`, **0 scritture**, nessuna sovrascrittura né «riparazione». I metadati copiati sulla ricevuta sono validati **prima** di ogni scrittura (titolo canonico non vuoto, `className` stringa canonica **oppure** `null`, `ownerUid` e campi identitari coerenti): nessun fallback e nessuna normalizzazione silenziosa. Gli id sono validati come document ID Firestore sui **byte UTF-8** (≤ 1500), incluso l'id concatenato, prima di costruire qualunque `DocumentReference`. Risposta sanitizzata `{ status: 'submitted' \| 'already_submitted' }`: nessun uid, contenuto o codice. Consumata dal client tipizzato `forceSubmitClient` in `VerificationsView`. Nessun listener, nessun polling, nessun nuovo indice, nessuna chiamata OpenAI. |
...
interface SubmissionDoc {
  ...
  // FORCE-SUBMIT-01 — server-only, scritto SOLO dalla callable forceSubmitSubmission
