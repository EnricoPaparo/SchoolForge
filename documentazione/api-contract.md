...
| Elimina consegna (M4-LIFE-02, ridefinita in M5-06B) | `deleteSubmissionData(submissionId, ownerUid, db)`: legge prima tutti i target, verifica l'ownership dai documenti eliminati, poi — **novità M5-06B** — esegue un preflight autorevole: se la correzione è **mai stata restituita** (esiste `correctionReturns/{id}` anche se nascosto, oppure `corrections.status == 'returned'`, oppure il mirror pubblico `correctionStatus == 'returned'` su submission/receipt) interrompe **prima di ogni scrittura** con «La consegna non può essere eliminata perché la correzione è già stata restituita allo studente.» (nessuna cancellazione parziale). Se eliminabile, elimina in ordine i `correctionEvents` (`where ownerUid== + correctionId==`), poi `corrections/{id}`, e per ultimi `submissionReceipts/{id}` e `submissions/{id}`. `correctionReturns` **non è mai eliminata** da questo flusso (la sua esistenza è già causa di blocco). Idempotente e ripetibile, chunk di `delete` a ≤400 mutazioni. Al successo, un solo `auditEvents` **non identificativo** (`action: 'submission.deleted'`, `targetId: verificationId`, `actorUid: ownerUid`, timestamp — mai studentUid/submissionId/risposte). Nessuna lettura Storage. Eliminabili: consegne senza correzione, correzioni `in_progress` e `completed` mai restituite, sia su verifica `active` sia `closed`. UI: azione icon-only in Consegne online su verifica `active` **o** `closed`, mostrata solo per una consegna non restituita (per una restituita il cestino è disabilitato con spiegazione accessibile), con conferma; il dialog spiega che se la verifica è ancora disponibile lo studente potrà rifarla, mentre una verifica chiusa resta chiusa. |
