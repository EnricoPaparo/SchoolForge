...
**Correzioni restituite allo studente (M4-02B)**: `studentCorrectionReturnsService.loadStudentCorrectionReturns` legge tutte e sole le proprie restituzioni visibili con un'unica query `collection('correctionReturns')` filtrata su `studentUid == uid` **e** `visibleToStudent == true`, ordinata `returnedAt desc` — mai una scansione client-side della collezione, mai un `getDoc` per verifica. Entrambi i filtri sono richiesti dalla stessa regola già usata per `publishedProjection` sopra: la `allow read` esistente su `correctionReturns` (M4-01, invariata da M4-02B) autorizza già esattamente questa combinazione di campi in `resource.data`, quindi non è stata necessaria alcuna modifica a `firestore.rules` — solo un nuovo indice composito `COLLECTION` (`studentUid` ASC, `visibleToStudent` ASC, `returnedAt` DESC) in `firestore.indexes.json`, verificato con un test Emulator dedicato (query ammessa con entrambi i filtri; negata se ne manca uno). Il workspace di lettura (`StudentCorrectionView`) fa un secondo tipo di lettura, un `getDoc` singolo per submissionId (`loadStudentCorrectionReturn`, usato solo dal pulsante "Ricarica" manuale) — stessa regola, stesso confine: se il docente ha appena nascosto la restituzione, la lettura è negata e trattata come "non più disponibile", mai come errore non gestito.
