...
**M5-QUALITY-07 — allowlist runtime nano + Luna (codice, nessun deploy).** Dopo la revisione umana M5-QUALITY-06 superata dal docente, l'allowlist runtime di `settings/aiConfig` accetta **due** coppie modello→listino, ciascuna obbligatoria e univoca: `gpt-5.4-nano-2026-03-17` → `v2-2026-07-17-hg-m5` (invariato) e `gpt-5.6-luna` → **`v5-2026-07-20-luna-dev`** (1.000.000 micro-USD/M input, 6.000.000 micro-USD/M output; `cached input` non conteggiato perché non misurato). Il parser resta fail-closed: modello non ammesso, listino non accoppiato (es. Luna con listino nano o nano con listino Luna), o versione sconosciuta ⇒ provider disabilitato prima del secret, del grader e di ogni prenotazione. Nessun fallback automatico tra modelli; nano resta scelta esplicita. Tutto il flusso costi/budget/idempotenza vale per Luna e `aiCorrectionRuns` registra `model`/`priceListVersion` effettivi; prompt, `promptContractVersion`, Structured Output, timeout, retry, privacy e singola chiamata per consegna invariati. La preview mostra token e costo stimati col listino del modello attivo. Il rollout DEV (kill switch → deploy → aggiornamento config → smoke di una consegna → conferma docente) e il rollback (`enabled=false`, poi ritorno esplicito a nano) avvengono **dopo il merge**: questa modifica non attiva provider, non tocca Firestore/secret e non esegue deploy. **G7 aperto** fino allo smoke reale DEV.
