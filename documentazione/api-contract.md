...
| `aiContentPreview` + `aiContentGenerate` (**AIGEN-01 implementato**) | Generazione IA contenuti | Due Function `onCall` v2 owner-only, scale-to-zero. Payload **chiuso discriminato** `kind: 'pool' \| 'lesson'` (mai model ID/listino/prezzi/budget/API key/system prompt; proprietà extra ⇒ `invalid_input`). `aiContentPreview` = stima token/costo senza provider/prenotazione/scrittura; `aiContentGenerate` = ordine fail-closed (auth→owner→kill switch→payload→dimensioni→profilo/modello/listino→stima/limiti→run/lease/idempotenza→budget→provider→validazione output→persistenza→riconciliazione→finalizzazione), **una** chiamata provider (retry ≤ 1), run server-only `aiContentRuns/{opaqueRunId}` (TTL 24h). AIGEN-01 valida solo la struttura semantica della proposta pool (nessun ID/`parsePool`; materializzazione `schoolforge-pool/v2` in AIGEN-02). Provider reale disabilitato dal kill switch. Contratto in [`ai-content-generation-roadmap.md`](ai-content-generation-roadmap.md). |
