...
| `assignVerificationVariant` (**VEX-01B, esteso da VDIF-04**) | Verifiche online | `onCall` v2, region `us-central1`, scale-to-zero. **Nessuna Function nuova**: VDIF-04 estende questa. Input chiuso `{ verificationId }` (proprietà extra / segmento Firestore non valido ⇒ rifiutati). Autorizzazione fail-closed invariata: auth, studente approvato dello stesso owner, classe della verifica, `active` + `onlineEnabled`, snapshot valido. Copre i **tre** casi risolvibili dal server — solo varianti equivalenti, solo differenziazione, entrambe — tramite `parseResolvableSnapshot`; una verifica che non richiede assegnazione dal server è rifiutata (`failed-precondition`). L'insieme è calcolato da `resolveDifferentiatedOrders`: comuni, poi sostituzioni e omissioni dell'etichetta **congelata** dello studente (`teacherSnapshot.labelAssignments`, mai la collezione live), poi **una** alternativa per gruppo VEX con RNG `node:crypto` sicuro e iniettabile; insieme vuoto o con duplicati ⇒ errore, mai un ripiego. **Transazione idempotente** invariata: unica scrittura `assignedQuestionOrders` + `assignedAnswerKeys` sulla submission deterministica, **0** scritture ai riaccessi; un'assegnazione persistita incoerente con l'etichetta congelata è fail-closed, mai rigenerata. Restituisce `{ assignmentMode: 'server_resolved', assignedQuestionOrders, questions[] }` sanitizzato — **mai** soluzioni, alternative non assegnate, `teacherSnapshot`, gruppi, etichette, nomi di etichetta o motivi della selezione. Il campo di risposta `distributionMode` è stato **sostituito** da `assignmentMode`: dice da quale canale arrivano le domande, non che natura abbia la verifica. Consumata dal client tipizzato `verificationVariantClient`; `same_questions` **senza** differenziazione non la invoca e resta interamente client-side. Nessun listener/polling; nessun documento per domanda; nessuna copia del pool. Contratti in [`vex-contract.md`](vex-contract.md) e [`verifiche-differenziate-roadmap.md`](verifiche-differenziate-roadmap.md) §20. |
| `resolveStudentVerificationPdf` (**PDF personale server-resolved**) | PDF verifica studente | `onCall` v2, `us-central1`, scale-to-zero. Input chiuso `{ verificationId }`; autorizzazione fail-closed su portale, studente approvato, owner, classe, stato `active/closed`, `visibility: public`, `studentPdfEnabled: true` e snapshot risolvibile. Restituisce lo stesso contratto sanitizzato di `assignVerificationVariant`, ma non crea né aggiorna `submissions`. La prima richiesta può fissare l'assegnazione in `verifications/{verificationId}/studentAssignments/{studentUid}` (server-only); svolgimento e PDF riusano la stessa assegnazione e una divergenza è rifiutata. Nessuna soluzione, alternativa non assegnata o etichetta raggiunge il browser. |
...
studente `server_resolved` passa da `resolveStudentVerificationPdf`, che
restituisce soltanto le domande personali e non crea una submission. Il PDF
docente completo resta invariato.
...
**M5-05E-1 — Human Gate e guardrail di costo approvati (stato storico).** Dal 17 luglio 2026 la configurazione nano pinned usa Responses API, Structured Outputs e listino `v2-2026-07-17-hg-m5`. Dall'11 settembre 2026 `settings/aiConfig` richiede interi positivi `maxOperationCostMicroUsd ≤ 5000000`, `dailyBudgetMicroUsd ≤ 15000000` e `monthlyBudgetMicroUsd ≤ 15000000`; campo mancante, invalido o sopra ceiling ⇒ fail-closed. Il tetto della singola operazione resta invariato, mentre giorno e mese possono sostenere il carico reale di creazione lezioni autorizzato dal docente. L'ordine del run reale è **auth/owner → config/kill switch → classificazione/limiti → costruzione grader e prenotazione conservativa → limite operazione → lease → transazione ledger giornaliera/mensile → provider**. L’approvazione era allora decisionale; Luna e la chiusura operativa sono documentate in M5-08.
...
Il dataset attivo è v4: gli holdout A, B e C hanno trovato difetti distinti e
sono archiviati byte per byte; il nuovo holdout D resta indipendente dal
candidato corretto. Il contratto della proposta e quello dell'immagine
richiedono che confronti ordinati usino la stessa origine, scala o linea di base
ed estremi distinti e direttamente confrontabili; traiettorie con origini
diverse non dimostrano una distanza ordinata senza un piano di arrivo comune.
