...
**M5-QUALITY-01 (implementato).** Il dialog aggiunge lo **stile di valutazione** `gradingMode` (`compassionate`/`balanced`/`rigorous`, default `balanced`) con descrizione dinamica, e porta il limite di `teacherGuidance` a **500** caratteri con contatore `0/500`. `gradingMode` attraversa tipizzato UI→client→preview→run→engine→prompt e sposta il punteggio solo entro la fascia giustificata dalle evidenze; entra nell'hash idempotente (cambio stile/indicazioni con stessa `requestId` ⇒ `invalid_input`, nuova `requestId` a ogni modifica); assente (proprietà omessa, `undefined`) ⇒ `balanced`, presente ma invalido — incluso `null` — ⇒ `invalid_input`. In stima è read-only con «Modifica impostazioni». Costi/idempotenza/contratto M4 invariati: una chiamata provider per consegna con aperte, zero per sole chiuse; nessuna nuova lettura/scrittura, listener o dipendenza; `gradingMode`/`teacherGuidance` non persistiti nei run/ledger/log. **M5-QUALITY-02 (benchmark) e Gate G7/M5 restano aperti.**

**M5-QUALITY-02 / M5-QUALITY-04-FIX (NEW REAL BENCHMARK REQUIRED).** Il terzo benchmark reale autorizzato ha completato e misurato 36/36 chiamate senza output invalidi (76.287 token, circa 0,03434 USD): la prompt injection è passata a revisione manuale, ma il verdict resta `AUTOMATIC_CHECKS_FAILED` per SCI-002, SCI-003 e SCI-004. M5-QUALITY-04-FIX aggiunge una micro-rubrica interna compatta: alternative valutate sul meccanismo causale, elementi scientifici sostanziali mai inferiti se assenti e penalizzazione significativa delle aggiunte false pertinenti su classificazioni o proprietà centrali. Modello, schema Structured Outputs, comparatore, range e guardrail restano invariati; nessuna chiamata reale è eseguita in questa PR. **M5-QUALITY-02 e Gate G7 restano aperti fino a un nuovo benchmark autorizzato e alla revisione docente.** Vedi [`evidenze/m5-quality-02-benchmark.md`](evidenze/m5-quality-02-benchmark.md).

**M5-QUALITY-05 (confronto modello controllato, solo benchmark).** Per capire se i fallimenti sistematici residui del Gate G7 (SCI-002/003/004) dipendano dal modello o dal contratto di valutazione, il benchmark accetta un override di modello **esclusivamente dalla CLI locale** (`--benchmark-model=<modello>`) con allowlist chiusa e fail-closed: solo `gpt-5.4-nano-2026-03-17` (baseline), `gpt-5.4-mini-2026-03-17` e `gpt-5.6-luna` (candidati); un modello diverso termina prima di leggere `OPENAI_API_KEY` e prima di qualunque rete. Il modello è passato esplicitamente all'`OpenAiGrader` del benchmark: **non** legge/modifica `settings/aiConfig`, **non** modifica `OPENAI_PRODUCTION_MODEL`, non cambia il modello runtime delle Functions e non persiste nulla. Listino esteso con due nuove versioni, una per candidato: `v3-2026-07-20-mini-benchmark` (mini: 0,75 USD/M input, 4,50 USD/M output) e `v4-2026-07-20-luna-benchmark` (Luna: 1,00 USD/M input, 6,00 USD/M output); `v2-2026-07-17-hg-m5` (nano) resta immutata; nessun `cached input` inventato. Report locali distinti per modello e ignorati da Git; sintesi comparativa che supporta **nano vs mini vs Luna** (baseline + almeno un candidato; candidati assenti elencati, mai inventati; non disponibile se manca il baseline). Uno stamp `promptContractVersion` (hash prompt+schema) su ogni report consente di riusare un report reale nano esistente come baseline solo se dataset/modello/listino/prompt coincidono. Confrontabilità rigorosa: stesso dataset/casi/modalità/ripetizioni/prompt/Structured Output/parametri/fasce e stesso numero pianificato di chiamate; **nessun** fix qualitativo al prompt in questa PR. Dry-run verificati (nessuna chiamata reale): nano tetto 834.350 µUSD, mini tetto 3.020.810 µUSD, Luna tetto 4.026.954 µUSD. Nessuna scelta definitiva del modello; produzione e DEV restano su nano; i benchmark reali mini e Luna restano subordinati ad autorizzazione esplicita del docente. **Gate G7 resta APERTO.**

**M5-QUALITY-06 (ricalibrazione docente INF-004 + candidatura G7).** Il benchmark reale Luna (autorizzato) ha completato 36/36 chiamate senza output invalidi (costo 176.235 µUSD, 81.130 token, latenza media 5.189,861 ms; `promptContractVersion` `db91d19cc4c43f2a`), risolvendo SCI-002/003/004; il Gate falliva ancora principalmente per **INF-004**. Il docente ha giudicato la fascia balanced 2,50–3,25 troppo generosa (mancano instradamento IP e l'intero nucleo TCP affidabilità/errori/ritrasmissioni) e l'ha ricalibrata a **2,00–2,50**, con derivazione mode-aware invariata (compassionate ≤ +0,50, rigorous ≤ −0,50, step 0,25, 0..maxPoints). È una revisione pedagogica esplicita: modificate **solo** la fascia e la motivazione di INF-004; domanda/soluzione/risposta/prompt/Structured Output/altre fasce/classificazione anomalie/modello runtime/listini/provider invariati. Aggiunta la **rivalutazione offline** (`benchmark:m5-quality:reevaluate`): ricostruisce i risultati grezzi dal report comparativo, ri-esegue il comparatore col dataset aggiornato preservando verbatim il blocco tecnico reale, senza rete/chiave/nuove risposte, senza sovrascrivere l'originale, fail-closed su datasetVersion/promptContractVersion/modello/struttura. Nel checkout dell'agente il report Luna non è presente: nuovo verdetto automatico **non disponibile**, non ricostruito né inventato (meccanica coperta da test e provata su report sintetico). Il vecchio nano non è confrontabile automaticamente (privo di `promptContractVersion`) e **non viene ripetuto**. **Gate G7 resta APERTO** in attesa della conferma umana su feedback per-domanda, feedback generali, resistenza alle prompt injection e accettabilità delle modalità; Luna resta **benchmark-only**, non promossa in DEV/runtime.

**M5-QUALITY-07 (promozione controllata di GPT-5.6 Luna nel runtime DEV).** La revisione umana M5-QUALITY-06 è **SUPERATA** dal docente (feedback per-domanda, feedback generali, resistenza alle prompt injection, accettabilità delle modalità e dei finding manuali SCI-009). Il codice runtime è esteso per supportare `gpt-5.6-luna` in DEV con nano fallback **esplicito** (nessun cambio automatico di `settings/aiConfig`, `OPENAI_PRODUCTION_MODEL`, Firestore o Secret). Allowlist runtime chiusa con accoppiamento obbligatorio modello→listino: nano→`v2-2026-07-17-hg-m5`, Luna→**`v5-2026-07-20-luna-dev`** (nuova versione runtime dedicata, 1.000.000 µUSD/1M input, 6.000.000 µUSD/1M output; `cached input` non conteggiato). Qualsiasi coppia incoerente (Luna+listino nano, nano+listino Luna, modello/listino sconosciuti) è respinta **fail-closed** dal parser prima del provider e di ogni prenotazione; nessun fallback silenzioso. Tutto il flusso costi/budget vale per Luna (preview, stima, tetto conservativo, prenotazione, mark pending, riconciliazione, recovery, hard stop, `aiCorrectionRuns` con modello/listino registrati, replay/idempotenza, takeover lease); invariante `costActual ≤ costSettled ≤ costReservation` verificata anche con Luna e Unicode. Prompt, `promptContractVersion`, Structured Output, teacher guidance, grading mode, timeout, retry, max output, validazione, privacy e singola chiamata per consegna invariati. UI invariata: la preview mostra Modalità OpenAI, token e costo stimati dal backend col listino del modello attivo. **Nessun deploy, nessuna modifica a Firestore, nessuna chiamata OpenAI in questa PR**: il rollout controllato (verifica config → `enabled=false` → deploy Functions → aggiorna `aiConfig` a Luna disabilitato → preview fail-closed → `enabled=true` → una consegna DEV → verifiche punteggi/feedback/runs/ledger/costo/PII → conferma docente) e il rollback (`enabled=false` immediato, poi ritorno esplicito a nano) avvengono dopo il merge. **Gate G7 non ancora chiuso**: PASS solo dopo lo smoke reale DEV e la conferma finale del docente.
