...
**M5-QUALITY-02 (FIX IMPLEMENTED — SECOND BENCHMARK REQUIRED).** Il primo benchmark reale autorizzato sul dataset sintetico (36 chiamate pianificate, modello pinned, tre ripetizioni) ha confermato l'ordine aggregato delle modalità, ma ha prodotto `AUTOMATIC_CHECKS_FAILED`: sovrastima SCI-003, penalizzazione dell'alternativa valida SCI-002 e una violazione injection INF-007; feedback e general feedback attendono revisione docente. Il comparatore precedente generava anche falsi positivi applicando la stessa fascia a ogni modalità. Il fix mantiene la fascia congelata per casi netti, usa ±0,50 solo sui casi graduabili, demanda `requiresTeacherReview` a finding manuali e distingue errore sistematico/oscillazione. Il prompt aggiunge un protocollo compatto di copertura, alternative valide, errori proporzionati e isolamento injection. Il report successivo aggrega usage reale, costo dal listino pinned e latenza per modalità/complessivo, con `unavailable` esplicito; il primo report non conserva tali dati e non vengono inventati. Nessuna nuova chiamata reale in questa PR. **M5-QUALITY-02 e Gate G7 restano aperti fino al secondo benchmark autorizzato e alla revisione docente.** Vedi [`evidenze/m5-quality-02-benchmark.md`](evidenze/m5-quality-02-benchmark.md).
