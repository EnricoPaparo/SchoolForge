...
**M5-QUALITY-05 (confronto modello controllato, solo benchmark).** Per capire se i fallimenti sistematici residui del Gate G7 (SCI-002/003/004) dipendano dal modello o dal contratto di valutazione, il benchmark accetta un override di modello **esclusivamente dalla CLI locale** (`--benchmark-model=<modello>`) con allowlist chiusa e fail-closed: solo `gpt-5.4-nano-2026-03-17` (baseline) e `gpt-5.4-mini-2026-03-17` (candidato); un modello diverso termina prima di leggere `OPENAI_API_KEY` e prima di qualunque rete. Il modello è passato esplicitamente all'`OpenAiGrader` del benchmark: **non** legge/modifica `settings/aiConfig`, **non** modifica `OPENAI_PRODUCTION_MODEL`, non cambia il modello runtime delle Functions e non persiste nulla. Listino esteso con la nuova versione `v3-2026-07-20-mini-benchmark` (mini: 0,75 USD/M input, 4,50 USD/M output); `v2-2026-07-17-hg-m5` (nano) resta immutata; nessun `cached input` inventato. Report locale distinto per modello e ignorato da Git; sintesi comparativa generabile solo con entrambi i report (altrimenti «non disponibile», senza inventare dati). Confrontabilità rigorosa: stesso dataset/casi/modalità/ripetizioni/prompt/Structured Output/parametri/fasce e stesso numero pianificato di chiamate; **nessun** fix qualitativo al prompt in questa PR. Dry-run verificati (nessuna chiamata reale): nano tetto 834.350 µUSD, mini tetto 3.020.810 µUSD. Nessuna scelta definitiva del modello; produzione e DEV restano su nano; il benchmark reale mini resta subordinato ad autorizzazione esplicita del docente. **Gate G7 resta APERTO.**
