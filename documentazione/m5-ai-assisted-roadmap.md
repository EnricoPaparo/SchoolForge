...
**Data:** 16 luglio 2026 · **Fase:** M5-00→M5-04C implementati; **M5-05A/B preparati**; **M5-05C implementato** con adapter e harness; **M5-05D1** (guardrail server-side), **M5-05D2A** (privacy/ciclo di vita run) e **M5-05D2B-1** (costo + budget mensile collegati al runtime) implementati, provider reale ancora disabilitato.
...
- **Ledger mensile atomico** (`aiBudgetLedger/{YYYY-MM}`, chiave UTC deterministica, server-only). La prenotazione avviene **atomicamente prima** della chiamata provider, è idempotente su `requestId` (retry/replay/concorrenza non raddoppiano), rifiuta `budget_exceeded` prima di qualsiasi chiamata, ha **hard stop al 100%** ed è recuperabile via scadenza (nessuno scheduler/polling). Due esecuzioni concorrenti non superano insieme il budget (transazione su singolo documento). La **riconciliazione** libera l'eccedenza e addebita l'effettivo, è idempotente e gestisce completed/partial/failed; verifica prima la titolarità della lease (`executionId`): un worker vecchio dopo un takeover non riconcilia né finalizza la prenotazione del nuovo worker.
- **Ordine sicuro del run:** auth/owner → config/kill switch → preflight/eleggibilità/limiti → stima costo → lease → **prenotazione budget** → chiamata provider → commit → **riconciliazione** → finalizzazione. `completed`/replay e `locked`/`conflict`/`legacy` non prenotano né chiamano il provider. Il clock di lease/prenotazione è letto al punto corretto (dopo preflight lenti).
- **`aiCorrectionRuns` v2** esteso con soli metadata aggregati (token e costo stimati/effettivi in micro-USD, versioni tecniche già ammesse); **nessun** ID/UID/PII/contenuto. Il ledger contiene solo importi tecnici e prenotazioni per `requestId` opaco. Replay invariato.
...
**Costi Firestore aggiunti.** Preview: **nessuna** lettura/scrittura oltre a M5-05D2A (una `get` di `settings/aiConfig` già esistente sul percorso reale; nessun accesso al ledger). Run reale con costo stimato > 0: **+1 transazione** di prenotazione e **+1 transazione** di riconciliazione sul **singolo** documento mensile (2 read + 1 write ciascuna, la riconciliazione legge anche il run doc per il gate di titolarità). Operazioni mock e sole-chiuse: **nessun** accesso al ledger. Replay/`locked`/`conflict`/`budget_exceeded`: **nessuna** transazione di budget. Nessun listener, polling, scheduler o risorsa sempre attiva: il recupero delle prenotazioni scadute avviene alla lettura successiva.
