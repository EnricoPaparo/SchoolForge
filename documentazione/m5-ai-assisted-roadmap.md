...
**Resta a M5-02:** eleggibilità reale per consegna (rilettura server-side di submission/`publishedProjection`/`teacherSnapshot`); scoring **deterministico** delle chiuse (incl. sole-chiuse) e valutazione delle **aperte** via `AiGrader`; scrittura delle `evaluations` nelle correzioni `in_progress`; **idempotenza persistita** e documento `aiCorrectionRuns/{requestId}`; stima token/costo reale nel preview. Provider reale, Secret Manager reale e budget: **M5-05** (Human Gate).
