...
Questi punti non erano decisi in M5-00. Il docente li ha approvati il **17 luglio 2026**; l'evidenza autoritativa è [evidenze/hg-m5-human-gate.md](evidenze/hg-m5-human-gate.md). In quel momento l’approvazione era solo decisionale; attivazione e G7 sono stati completati successivamente.
...
**Stato storico al termine di M5-02:** configurazione/attivazione reale su DEV, benchmark e budget erano demandati ai pacchetti successivi; sono ora completati e consolidati da M5-08.
...
**Stato storico al termine di M5-03:** attivazione reale, benchmark, budget e smoke erano demandati ai pacchetti successivi; M5-08 ne registra il completamento.
...
**Stato storico al termine di M5-04:** attivazione reale, benchmark e budget erano demandati ai pacchetti successivi; M5-08 ne registra il completamento.
...
**Stato storico al termine di M5-04C:** attivazione reale, benchmark e budget erano demandati ai pacchetti successivi; M5-08 ne registra il completamento.
...
**Stato storico di M5-05C:** adapter e harness erano stati consegnati con provider disabilitato. Attivazione, benchmark e guardrail sono stati completati nei pacchetti successivi.

### 16.8 Stato storico M5-05D1 — introduzione dei guardrail server-side

Prima parte di M5-05D: i **guardrail server-side obbligatori** che precedono qualsiasi attivazione del provider reale. In questa fase storica il provider era disabilitato; i guardrail sono rimasti invariati durante l’attivazione successiva. Mock e modalità `disabled` restano deterministici a costo zero; le domande chiuse restano a 0 token/costo.
...
- `expireAt` server-generated a 30 giorni DEV tramite clock iniettato. HG-M5-4 ha poi confermato definitivamente questa retention.
...
**Stato storico al termine di D2A:** retry, budget e attivazione erano demandati ai pacchetti successivi; M5-08 ne registra la chiusura.
...
**Stato storico al termine di D2B-1:** retry applicativo, benchmark e attivazione erano demandati ai pacchetti successivi; M5-08 ne registra la chiusura.
...
**Stato al termine di D2B-2:** benchmark, smoke e decisioni HG erano ancora da completare. Le decisioni sono state poi approvate in E-1 e l’intero percorso è stato chiuso da M5-08.
...
- **Config fail-closed:** `maxOperationCostMicroUsd` ≤ 5.000.000, `dailyBudgetMicroUsd` ≤ 15.000.000, `monthlyBudgetMicroUsd` ≤ 15.000.000; tutti obbligatori, interi e positivi. Assenza, tipo errato, zero o superamento ceiling invalidano l’intero documento. Il riallineamento dell'11 settembre 2026 triplica i tetti giorno/mese autorizzati per la creazione reale di lezioni, lasciando invariato il massimo per singola operazione.
...
**M5-QUALITY-01 (implementato).** Il dialog aggiunge lo **stile di valutazione** `gradingMode` (`compassionate`/`balanced`/`rigorous`, default `balanced`) con descrizione dinamica, e porta il limite di `teacherGuidance` a **500** caratteri con contatore `0/500`. `gradingMode` attraversa tipizzato UI→client→preview→run→engine→prompt e sposta il punteggio solo entro la fascia giustificata dalle evidenze; entra nell'hash idempotente (cambio stile/indicazioni con stessa `requestId` ⇒ `invalid_input`, nuova `requestId` a ogni modifica); assente (proprietà omessa, `undefined`) ⇒ `balanced`, presente ma invalido — incluso `null` — ⇒ `invalid_input`. In stima è read-only con «Modifica impostazioni». Costi/idempotenza/contratto M4 invariati: una chiamata provider per consegna con aperte, zero per sole chiuse; nessuna nuova lettura/scrittura, listener o dipendenza; `gradingMode`/`teacherGuidance` non persistiti nei run/ledger/log.

**M5-QUALITY-02 / M5-QUALITY-04-FIX (stato storico).** Il terzo benchmark nano aveva completato e misurato 36/36 chiamate senza output invalidi (76.287 token, circa 0,03434 USD), ma restavano i difetti SCI-002/003/004. La micro-rubrica introdotta da M5-QUALITY-04-FIX è rimasta nel contratto; la successiva valutazione Luna ha superato questi blocchi. Vedi [`evidenze/m5-quality-02-benchmark.md`](evidenze/m5-quality-02-benchmark.md).

**M5-QUALITY-05 (confronto modello controllato, solo benchmark).** Il runner benchmark accetta override solo dalla CLI locale con allowlist chiusa e fail-closed per nano, mini e Luna. I report locali restano ignorati da Git e `promptContractVersion` impedisce confronti fra contratti diversi. Il vecchio report nano, privo di tale versione, non è confrontabile automaticamente con Luna e non è stato ripetuto per evitare chiamate e costi non necessari.

**M5-QUALITY-06 (ricalibrazione docente INF-004).** Il benchmark reale Luna ha completato 36/36 chiamate senza output invalidi (176.235 µUSD, 81.130 token, latenza media 5.189,861 ms; `promptContractVersion` `db91d19cc4c43f2a`) e ha risolto SCI-002/003/004. Il docente ha ricalibrato esplicitamente INF-004 da 2,50–3,25 a 2,00–2,50 balanced; la rivalutazione offline ha prodotto `READY_FOR_MANUAL_REVIEW`, con tutti i criteri automatici obbligatori superati e zero anomalie automatiche bloccanti.

**M5-QUALITY-07 (promozione controllata di GPT-5.6 Luna nel runtime DEV).** La revisione umana è superata. L’allowlist runtime accoppia Luna a `v5-2026-07-20-luna-dev` e nano a `v2-2026-07-17-hg-m5`, senza fallback silenzioso. Il rollout controllato è stato eseguito: kill switch spento, deploy delle sole Functions IA, config Luna ancora disabilitata, abilitazione esplicita e smoke su una consegna DEV. Il docente ha dichiarato il test funzionale finale “impeccabile”.

**M5-08 — chiusura formale.** Con le evidenze automatiche, il benchmark reale, la rivalutazione offline e la conferma docente, **Gate G7 = PASS** e **M5 = COMPLETATO**. Luna è il modello DEV approvato; nano resta rollback esplicito. Evidenza completa: [`evidenze/g7-m5-checklist-finale.md`](evidenze/g7-m5-checklist-finale.md). G8 e VEX restano separati e non implementati.
...
- **M5-05C:** adapter OpenAI e harness testabili senza rete; `openai` fail-closed senza modello/secret; Structured Outputs + validazione applicativa; timeout 60 s e retry massimo 1 senza moltiplicazione SDK.
- **M5-05D1:** config runtime `settings/aiConfig` fail-closed e unica fonte del modello; ordine auth/owner → config/kill switch → classificazione/limiti → secret/grader → lease; ceiling DEV non aumentabili; mock e sole-chiuse invariati a 0 token/costo.
...
- **M5-05D2B-1:** costo versionato e ledger mensile atomico; prenotazione conservativa crash-safe `reserved → pending`, riconciliazione gated dalla lease, usage validato e run/ledger senza ID/UID/PII. I ceiling successivi sono ora approvati e attivi.
- **M5-05D2B-2:** retry applicativo unico (`SDK maxRetries: 0`), ≤ 1 retry sui soli transitori, `Retry-After`/backoff/jitter/deadline limitati; prenotazione comprensiva dei tentativi e telemetria aggregata privacy-safe.
...
- **Human Gate HG-M5-1/2/3/4 approvati e Gate G7 PASS:** provider Luna operativo su DEV dietro kill switch e guardrail; nano resta rollback esplicito.
- **Roadmap:** M5-00→M5-08 completati con **G7 PASS**. **G8/automatica e VEX restano fasi separate.**
