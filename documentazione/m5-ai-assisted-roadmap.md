...
**M5-QUALITY-02 (READY FOR MANUAL BENCHMARK).** L’harness esistente esegue ora gli stessi raggruppamenti sintetici in `compassionate`, `balanced` e `rigorous`, con ripetizioni configurabili e grader iniettato. Un comparatore puro produce risultati per occorrenza/modalità, totali, differenze, anomalie e verdetti automatici/manuali secondo la rubrica, senza includere domanda, soluzione o risposta nel report. Il runner locale mostra prima un piano dry-run con upper bound di chiamate/token/costo; il percorso reale richiede flag espliciti e conferma interattiva. Nessuna chiamata reale è stata eseguita in questo pacchetto: risultati pedagogici, costi e Gate G7 restano subordinati al benchmark autorizzato e alla revisione docente. Vedi [`evidenze/m5-quality-02-benchmark.md`](evidenze/m5-quality-02-benchmark.md).
