...
**M5-QUALITY-02 / M5-QUALITY-03-FIX (NEW REAL BENCHMARK REQUIRED).** Il secondo benchmark reale autorizzato ha completato e misurato 36/36 chiamate (77.420 token, circa 0,041389 USD), ma ha confermato `AUTOMATIC_CHECKS_FAILED`: un output `rigorous` invalido, alternativa valida SCI-002 penalizzata, SCI-003 sistematicamente sovrastimata, SCI-004 troppo generosa e una oscillazione injection INF-007. M5-QUALITY-03-FIX rafforza in modo compatto checklist di copertura, alternative semanticamente valide, proporzionalità delle risposte parziali, penalizzazione delle aggiunte false, isolamento injection ed esatta copertura degli `order`; il benchmark registra solo `reasonCode` sanitizzati (`schema_invalid`, `missing_result`, `invalid_score`, `invalid_general_feedback`, `provider_error`), mai raw output o contenuti. Modello, schema Structured Outputs, comparatore e guardrail restano invariati; nessuna chiamata reale è eseguita in questa PR. **M5-QUALITY-02 e Gate G7 restano aperti fino a un nuovo benchmark autorizzato e alla revisione docente.** Vedi [`evidenze/m5-quality-02-benchmark.md`](evidenze/m5-quality-02-benchmark.md).
