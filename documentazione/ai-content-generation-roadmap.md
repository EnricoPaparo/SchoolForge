...
> **LESSON-MANUAL-02 progettato** (solo documentazione, nessuna generazione): sei scenari congelati coprono teoria introduttiva, procedura tecnica, esempi pratici, esercizi svolti, contenuto avanzato e confine UDA; la rubrica misura 15 dimensioni su scala 0–4 con blocker e soglie esplicite; il protocollo separa difetti di prompt, renderer, metadati e variabilità del modello. Baseline primaria `economy`, una generazione per scenario solo dopo stima e nuova autorizzazione. Verdetto iniziale: **NON DISPONIBILE**, perché non sono stati prodotti campioni né sostenuti costi. Nessun prompt, modello, payload, listino, budget o runtime è stato modificato.

> **LESSON-MANUAL-03 implementato, solo runner locale protetto; nessuna chiamata reale:** `lessonManualQualityBenchmark.ts` carica e valida fail-closed le versioni congelate, converte i 6 scenari nel payload lezione autorevole e costruisce il piano riusando modello/listino, prompt, Structured Output, retry e formule di costo AIGEN; `lessonManualQualityCli.ts` è dry-run di default e consente il percorso reale solo con due flag, Node 22, TTY, frase esatta e chiave letta per ultima. Output originali e report sono locali in `functions/lib/` (gitignored), zero Firestore/Storage. Dry-run: 6 chiamate, ≤12 tentativi, stima 78.698 µUSD, tetto 169.910 µUSD. Verdetto qualità ancora **NON DISPONIBILE**.
>
> **LESSON-TUNE-01 implementato, nessuna chiamata reale:** estende senza mutare LESSON-MANUAL-02 a 12 scenari e congela uno split anti-overfitting (8 `tuning`, 4 `holdout`). `lessonTuneQualityBenchmark.ts` valida e combina dataset base/estensione, mentre `lessonTuneQualityCli.ts` registra `AI_CONTENT_PROMPT_VERSION`, è dry-run di default e vieta l'esecuzione reale senza scegliere esplicitamente uno split. Il protocollo impone che gli holdout siano eseguiti una sola volta dopo il congelamento del candidato. Dry-run complessivo: 12 chiamate, ≤24 tentativi, stima 162.393 µUSD, tetto 349.894 µUSD; tuning: 94.084/204.710 µUSD; holdout: 68.309/145.184 µUSD. Zero secret/provider/costi reali.
>
