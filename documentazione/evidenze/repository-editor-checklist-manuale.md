...
1. Un account che è owner del progetto, con accesso alla sezione **Didattica** (`TeacherShell`).
...
|---|---|---|---|---|---|
| 1 | Creare programma/import base | In **Didattica**, crea un nuovo programma (titolo libero), poi "Importa ZIP" con lo ZIP minimo del setup | La card del programma compare con l'import attivo; aprendo il corso, UDA e lezione dello ZIP sono visibili nel workspace | `activeImportId` impostato; nessun errore di validazione; UDA/lezione visibili nel workspace Didattica | ⬜ da eseguire |
| 2 | Creare UDA | In **Didattica**, apri il corso appena creato e scegli "Nuova UDA"; compila Titolo (obbligatorio) e opzionalmente Descrizione/Competenze/Obiettivi; "Crea UDA" | La nuova UDA compare subito nel workspace, senza dover ricaricare la pagina | Nessun refetch necessario; numerazione tecnica (`uda-XX-slug`) coerente col numero massimo esistente + 1 | ⬜ da eseguire |
...
| 4 | Modificare metadata | Apri la lezione creata al punto 3, vai alla scheda **Informazioni** e scegli "Modifica informazioni"; cambia titolo/sottotitolo/difficoltà/concetti chiave/obiettivi; "Salva" | Il pannello si chiude e mostra i nuovi valori | Nessun errore mostrato; riapri il form e verifica che i valori salvati siano quelli appena inseriti (non quelli precedenti) | ⬜ da eseguire |
...
| 6 | Riordinare UDA/lezioni | Con almeno 2 UDA (e 2 lezioni nella stessa UDA) presenti, attiva **Organizza UDA** a livello corso o **Organizza lezioni** a livello UDA; usa "▲"/"▼" per scambiare un elemento con il vicino | L'ordine cambia immediatamente dopo il click, senza refetch | "▲" è disabilitato sul primo elemento della lista, "▼" sull'ultimo; durante il salvataggio i controlli non permettono doppi click; scegli "Fine" per uscire dalla modalità Organizza | ⬜ da eseguire |
| 7 | Eliminare lezione libera | Con Organizza **non** attivo, apri la lezione creata al punto 3 e dal menu contestuale scegli "Elimina lezione"; nel dialog di conferma, click "Elimina definitivamente" | La lezione scompare dal workspace; se era selezionata, il workspace torna al livello UDA | Nessun messaggio di blocco; se si riapre il corso/UDA con un refresh pagina, la lezione eliminata non ricompare (Firestore/Storage puliti, non solo stato locale) | ⬜ da eseguire |
| 8 | Tentare eliminazione bloccata da verifica | In **Verifiche**, crea una nuova verifica in bozza sullo stesso programma/import e aggiungi almeno una domanda dal pool di un'altra lezione. Torna in **Didattica**, apri quella lezione (o la sua UDA), scegli l'eliminazione e conferma | L'eliminazione **non avviene**: il dialog mostra l'elenco delle verifiche bloccanti (titolo + stato, es. "bozza") invece del pulsante di conferma | La lezione/UDA resta visibile dopo il tentativo; nessun file è rimosso da Storage (riaprendo la lezione il contenuto è ancora presente); chiudi il dialog di blocco | ⬜ da eseguire |
| 9 | Esportare ZIP | In **Didattica**, apri il programma usato ai punti precedenti e dal menu del corso scegli "Esporta ZIP" | Il browser scarica un file `<titolo_programma>_export.zip` | Estrai il ZIP: contiene le cartelle `uda-XX-slug/` con i file `.md` di UDA e lezioni correnti (non quelli eliminati al punto 7), niente file `.pool.md`; l'ordine delle cartelle nell'archivio riflette l'ordine (`order`) visto dopo il riordino del punto 6, non l'ordine di creazione | ⬜ da eseguire |
| 10 | Reimportare ZIP in nuovo programma | In **Didattica**, crea un secondo programma nuovo (titolo diverso); "Importa ZIP" usando il file scaricato al punto 9 | L'import va a buon fine senza errori di validazione | UDA e lezioni del nuovo programma hanno lo stesso ordine visto nel programma originale dopo il riordino (punto 6); metadata e corpo Markdown coincidono con le modifiche fatte ai punti 4–5 | ⬜ da eseguire |
...
- **Creazione UDA/lezione**: `repositoryEditorService.test.ts` (`createUda`/`createLesson`), `CourseWorkspace.test.tsx` (flussi DUX-04A/04B).
- **Modifica metadata/corpo**: `repositoryEditorService.test.ts` (`updateUdaMetadata`/`updateLessonMetadata`/`updateLessonMarkdownBody`), `CourseWorkspace.test.tsx` (schede Contenuto/Informazioni).
- **Riordino**: `repositoryEditorService.test.ts` (`reorderUda`/`reorderLesson`, incl. fallback legacy senza `order`), `CourseWorkspace.test.tsx` (DUX-04C, inclusi limiti e busy-lock).
- **Eliminazione protetta**: `repositoryEditorGuards.test.ts` (guardia pura `findRepositoryDeleteBlockers`), `repositoryEditorService.test.ts` (`deleteUda`/`deleteLesson`, blocco/libera, tolleranza a file Storage assenti), `CourseWorkspace.test.tsx` (conferma esplicita, elenco blocchi, aggiornamento selezione).
