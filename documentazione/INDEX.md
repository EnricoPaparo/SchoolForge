...
**Hardening finale (HARD) — HARD-00 audit svolto.** Audit generale finale V1 read-only completato: verdetto **READY FOR REMEDIATION** (0 P0, 0 P1, 3 P2, 5 P3), nessun codice/Rules/config modificato — vedi [hardening-audit-v1.md](hardening-audit-v1.md). **HARD-01A completato:** runbook operativo [runbook-operativo-v1.md](runbook-operativo-v1.md) e Human Gate [evidenze/hard-01a-human-gate.md](evidenze/hard-01a-human-gate.md) — finding HARD-F01 **RESOLVED (15/07/2026)**. **HARD-01B completato:** security header e strategia cache di Hosting in `firebase.json`, deployati e verificati su DEV — finding HARD-F03 **RESOLVED (15/07/2026)** (evidenze [evidenze/hard-01b-dev-smoke.md](evidenze/hard-01b-dev-smoke.md), 12/12 PASS). **HARD-01C:** matrice regioni e residenza dati ([evidenze/hard-01c-region-matrix.md](evidenze/hard-01c-region-matrix.md)); corrette le affermazioni «tutto a Milano `europe-west8`» con lo stato reale (DEV Storage/Function `us-central1`; Firestore DEV **`europe-west8` verificata**; target PROD **UE**) — finding HARD-F02 **MITIGATED: resta la decisione sulla regione PROD** ([evidenze/hard-01c-human-gate.md](evidenze/hard-01c-human-gate.md)). **HARD-02A svolto:** audit accessibilità end-to-end docente+studente ([evidenze/hard-02a-a11y-audit.md](evidenze/hard-02a-a11y-audit.md)) — **READY FOR REMEDIATION** (0 P0, 0 P1, 1 P2: `DialogShell` senza Escape/focus-trap/restore; P3 di polish). **HARD-02A-FIX completato:** Escape/focus-trap/restore centralizzati in `DialogShell` — **P2-01 RESOLVED**, smoke a11y manuale su DEV **PASS (15/07/2026)**. **HARD-02B-00 design-ready** ([hard-02b-import-chunking-design.md](hard-02b-import-chunking-design.md)). **HARD-02B-1 implementato:** fondazione — ID `publicLessons` import-scoped (`LessonDoc.publicLessonId` + helper), query studente `programId + activeImportId`, Rules obbligatorie `importId == activeImportId` (non-owner), indice `publicLessons(programId, importId)`; transazione import ancora atomica. **HARD-02B-2 completato:** chunking ≤400 (helper condiviso `firestoreChunks.ts` riusato da `poolEditorService`), macchina a stati `staging→active→superseded`, switch atomico (`activeImportId`+`imports/{id}.status='active'`+audit), cleanup differito delle sole `publicLessons` del vecchio import (`stalePublicLessonsCleanup.ts`, idempotente + retry), risultato `committed`+`cleanupPending`/`not_applied` — finding **HARD-F06 RESOLVED (15/07/2026)**. I restanti pacchetti HARD (03) e il Gate GHARD **non sono ancora implementati**.
...
- [evidenze/hard-01c-region-matrix.md](evidenze/hard-01c-region-matrix.md) — **HARD-01C** matrice regioni DEV/PROD (Firebase/Auth/Firestore/Storage/Functions/Hosting) con stato, regione verificata e fonte; strategia residenza dati e correzioni documentali. HARD-F02 **MITIGATED**.
- [evidenze/hard-01c-human-gate.md](evidenze/hard-01c-human-gate.md) — **HARD-01C** Human Gate: azioni manuali residue del docente (verifica regione Firestore DEV, decisione regione UE per PROD, autorizzazione provisioning ex novo). Stato iniziale: `PENDING`.
