...
16. [verifiche-differenziate-roadmap.md](verifiche-differenziate-roadmap.md) — verifiche differenziate per etichetta (VDIF) ed esiti per lezione (ESITI-01, sola lettura, derivazione pura). **VDIF-00 completato**: contratto tecnico congelato, perimetro privacy a elenco chiuso, mutua esclusione VEX bidirezionale, guardie di attivazione, cost model, matrice di test e prototipo [prototipi/verifiche-differenziate.html](prototipi/verifiche-differenziate.html). Nessuna riga di codice runtime implementata. **Gate GVDIF aperto.**
17. [mappa-concettuale-roadmap.md](mappa-concettuale-roadmap.md) — mappa concettuale della lezione: output IA strutturato e composto dal server, Markdown modificabile, diagramma a caratteri e proiezione studente presente solo a lezione svolta. **CONCEPT-MAP-01→07 implementati**: artefatto **v2** Sintesi + Diagramma, lettura compatibile delle mappe v1 e generazione modale con scelta esplicita Economy/Quality (default Quality); tuning reale della sintesi concluso con `concept-map-07-v1`. **Il gate umano resta aperto fino allo smoke reale.**
18. [pool-quality-roadmap.md](pool-quality-roadmap.md) — protocollo anti-overfitting per misurare e migliorare i pool IA: 12 fonti congelate (8 tuning + 4 holdout), confronto Economy/Quality, rubrica 10×0–4, blocker e runner protetto. **POOL-TUNE-00→03 completati; Gate GPOOL-QUALITY PASS esclusivamente per `pool-tune-02-candidate-a-v1` + profilo `quality`** (Economy resta non qualificato). **POOL-ROLLOUT-01 implementato nel codice; il rollout DEV resta da eseguire.**
