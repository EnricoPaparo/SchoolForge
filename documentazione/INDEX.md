...
17. [mappa-concettuale-roadmap.md](mappa-concettuale-roadmap.md) — mappa concettuale della lezione: output IA strutturato e composto dal server, Markdown modificabile, diagramma a caratteri e proiezione studente presente solo a lezione svolta. **CONCEPT-MAP-01, 02, 03 e 04 implementati** (core e backend IA; persistenza, proiezione condizionale e Rules; interfaccia docente e studente; mappa come **scheda** della lezione — docente Contenuto → Mappa concettuale → Domande → Informazioni, studente Contenuto → Mappa concettuale solo quando la mappa è davvero proiettata). **Rollout DEV e gate umano ancora aperti**: nessun deploy, nessuna generazione OpenAI reale.
