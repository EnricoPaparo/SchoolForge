...
| [storage-gateway-roadmap.md](storage-gateway-roadmap.md) | Contratto e roadmap del **Repository Storage Gateway** same-origin (SGW-00–03): `/api/repository/*` → Hosting rewrite → Cloud Function → Admin SDK → Storage, per rendere pool/import/export/editing/verifiche affidabili anche su Brave. **Stato: TARGET approvato, NON ancora implementato.** | Prima di iniziare SGW-01, o per capire perché lo Storage diretto è a rischio su Brave. |
| [didattica-ux-roadmap.md](didattica-ux-roadmap.md) | Specifica UX del redesign "Didattica" (DUX-00–07 + Gate GDUX). **DUX-01–07B completati: viste legacy rimosse, Classi assorbita in Studenti, Verifiche uniformata, polish responsive e metadata corso applicati; Gate GDUX pendente.** | Per consultare architettura informativa, invarianti di sicurezza/costo e prossimo Gate. |
