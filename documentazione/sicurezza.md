...
- **Modalità verifica (M3F-07) eredita esattamente questo stesso limite residuo — non lo introduce, ma lo rende temporaneamente più visibile.** `firestore.rules` nega la *discovery* di `programs`/`publicLessons` a uno studente la cui classe è coperta da `settings/studentAccess.examMode` (funzione `examModeAppliesToClass`, § "M3-full" più sotto), esattamente come nega già l'assenza di approvazione/portale disattivato. Ma per lo stesso motivo spiegato sopra, Storage non ripete alcun controllo: un client che avesse già ottenuto (o indovinato) un `contentPath` prima dell'attivazione della Modalità verifica potrebbe continuare a leggerlo direttamente da Storage, bypassando il nuovo blocco. **M3F-07 non è quindi distribuibile da sola**: la UI docente dichiara la funzione disponibile, ma il blocco reale ed effettivo del contenuto lezione richiede M3F-08 (proiezione `publicLessons.content`, che sposta il corpo Markdown dentro Firestore stesso, l'unico servizio su cui Modalità verifica ha già effetto). Fino a quel momento, "Modalità verifica attiva" va inteso come "le lezioni sono nascoste e non più scopribili dall'app", non come "il contenuto è tecnicamente irraggiungibile" — la stessa distinzione già valida per l'intera sezione Lezioni sopra.
