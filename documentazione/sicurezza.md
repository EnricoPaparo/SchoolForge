...
**Versione:** 3.0
**Stato:** in vigore — controlli implementati da F-04 in avanti (M1, M2, M3-lite, RE, M3-full); M3-full completato con Gate G5 superato (vedi `documentazione/evidenze/g5-m3-full-checklist-finale.md`); M4-00→M4-03A implementati, incluso Registro Correzioni ed export CSV locale owner-only — PDF/Markdown, Gate G6 e M5 restano non completati/rinviati
...
- **Export Registro Correzioni (M4-03A)**: disponibile solo nella `TeacherShell` e costruito dalle righe owner-only già presenti nel monitor, senza nuove query/listener/letture/scritture. Il modello CSV ammette esclusivamente nome, email, stato, riepilogo punteggio, data e codice consegna; non possiede campi per UID, submissionId, ownerUid, risposte, soluzioni, feedback o eventi. I campi testuali che iniziano con prefissi formula (`=`, `+`, `-`, `@`) vengono neutralizzati prima dell'escaping CSV per evitare formula injection in Excel. Nessuna modifica alle Security Rules: il download è un Blob locale non persistito.
