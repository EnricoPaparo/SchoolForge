...
**CONCEPT-MAP-02 — persistenza e visibilità della mappa.** La mappa vive in due copie: `LessonDoc.conceptMapMarkdown` (autorevole e owner-only) e `PublicLessonDoc.conceptMapMarkdown` (proiezione studente). Finché `completed !== true` il campo pubblico non deve esistere: Rules e lettura applicativa difendono questo confine anche contro un `get()` diretto. Salvataggio e cambio svolta/non svolta sono transazionali. La pubblicazione legge e valida la mappa privata; la smarcatura non la interpreta e la conserva anche se malformata, perché nascondere la proiezione deve restare sempre possibile. Entrambi i versi confrontano il fingerprint del valore privato grezzo, quindi una race non può lasciare una proiezione incoerente. Una mappa malformata blocca soltanto la pubblicazione. Owner, import, programma e identità stabile `udaDir`/`path`/`filename` sono sempre dimostrati. Le Rules applicano un tetto di 32.000 caratteri, mentre il limite autorevole applicativo resta 32.000 byte UTF-8. Nessun indice, documento o accesso Storage aggiuntivo.
...
**VISUAL-ENRICHMENT-01 — fase testuale dell'arricchimento visuale.** Nessuna immagine viene prodotta e nessun provider di immagini esiste ancora: questa fase aggiunge soltanto contratti puri, e il perimetro di sicurezza che introduce è quello del **payload** e del **soggetto**. Il payload è chiuso a undici chiavi e non trasporta dati studente, classi, etichette, UID, URL, riferimenti Storage né hash dichiarati dal client — `sourceBodyHash` sarà calcolato server-side dal `lessonBody` reale, perché un hash che arriva dal chiamante non dimostra nulla su ciò che il chiamante ha davvero mandato. Il corpo della lezione è **dato non attendibile**, delimitato con lo stesso `fence()` degli altri kind, e il preambolo dichiara esplicitamente che metadati, indice e corpo sono dati e non istruzioni. **Il `subject` è il collo di bottiglia, ed è per questo che ha un contratto proprio.** Secondo la roadmap §9.2 sarà l'unico testo variabile che raggiungerà il provider di immagini: il corpo della lezione non lo raggiungerà mai. È quindi l'unico canale per cui un'injection annidata nel corpo potrebbe propagarsi, e il filtro rifiuta fail-closed imitazioni di artisti viventi, studi e marchi, persone riconoscibili o identificabili, tentativi di ignorare le istruzioni o di sostituire il preambolo SchoolForge, concetti dichiaratamente assenti dalla lezione, e testo esteso, loghi, firme o watermark. Lo stesso confine valida anche l'allowlist testuale destinata all'immagine: massimo 8 etichette distinte fra caporali, massimo 40 code point, forma bilanciata e duplicati esatti deduplicati. Non esiste più una proposta valida che diventa invalida soltanto entrando nella fase immagini. La difesa strutturale resta però la più importante: il prompt immagine è **composto dal server** e un essere umano approva il soggetto prima che costi qualcosa. **I messaggi d'errore non replicano mai il soggetto integrale**, solo la categoria: i log di un tentativo di injection sono esattamente il posto in cui quel testo non deve finire. Il profilo è `quality` per contratto e `economy` è rifiutato prima di provider, prenotazione e scritture. Nessuna Rule, indice, collezione, campo Firestore, modifica alla CSP o alla sanificazione deriva da questo contratto; **Gate GVISUAL resta PENDING**.

**VISUAL-ENRICHMENT-02 — confine binario.** Due callable owner-only accettano il payload chiuso `{ requestId, subject }`. `aiVisualPreview` è read-only e non ha binding del secret; `aiVisualGenerate` è l'unica Function legata al secret Firebase `OPENAI_API_KEY`. La chiave viene letta soltanto nel ramo runtime `openai`, dopo mode, autenticazione, validazione, lease, budget e `reserved→pending`; non viene letta all'import, in preview, in mock o nei test. Modalità sconosciute/default ⇒ `disabled`; `openai` senza secret fallisce sanitizzato pre-provider e non degrada a mock. Il prompt provider contiene soltanto preambolo server-owned, allowlist di testo derivata dalle frasi `«…»` del `subject` e il `subject`; nessun UID, requestId, lessonId, corpo lezione o dato studente. La allowlist ha massimo 8 etichette **distinte** da 40 code point: ripetizioni esatte sono deduplicate senza ampliare il vocabolario, mentre una forma ambigua fallisce prima del provider. Il preambolo vieta di inventare oggetti, sostanze, azioni, istruzioni, relazioni o frecce non nominate e vieta qualunque testo fuori allowlist; nella versione `schoolforge-sketch-prompt/v5` un ordine stretto richiede la stessa origine, scala o linea di base ed estremi distinti. Traiettorie da origini diverse non dimostrano distanze ordinate senza un piano di arrivo comune esplicito. Un confronto non eseguibile senza dettagli inventati viene omesso. La versione fa parte della config chiusa del run, quindi il replay non attraversa silenziosamente un cambio di policy. Il provider è iniettato e i test usano mock/transport fake senza rete. I byte sono verificati indipendentemente dal MIME dichiarato, ricodificati WebP senza metadati e limitati a 1200 px/204.800 byte; hash e dimensioni sono misurati sul finale. `visualRuns/**` e `staging/**` sono negati a ogni client. Il parser closed-world riverifica i byte inline prima del replay, che non può riaddebitare o riusare il provider. `reserved` scaduto è liberabile; `pending` scaduto è addebitato al tetto. Un esito incerto dopo l'upload non viene ritentato: TTL e trigger idempotente eliminano esclusivamente lo stagingRef canonico. Nessun campo `LessonDoc`/`publicLessons`, nessun deploy e nessuna chiamata OpenAI reale in questa fase.

**Trasporto callable e autorizzazione.** Le callable visuali invocate dal web dichiarano `invoker: 'public'` perché Cloud Run deve accettare la richiesta della SDK Firebase prima che il token possa essere verificato dal runtime callable. Questo non rende pubblica l'operazione applicativa: ogni handler esegue immediatamente `requireOwner`, richiede `request.auth.uid` e lo confronta con `settings/owner`; un anonimo o un utente diverso viene quindi rifiutato prima della logica di dominio. Il trigger di cleanup TTL non è una callable client-facing e non adotta l'invoker pubblico. Un test strutturale congela entrambi i lati del confine. Senza l'invoker di trasporto, Cloud Run risponderebbe prima dell'handler e la UI vedrebbe una risposta mancante anche con un token Firebase valido.
...
studente. Mappa e manifest privati malformati non possono impedire questa messa
in sicurezza: su `true→false` non vengono interpretati né modificati, ma il loro
fingerprint grezzo resta nel controllo di race. La rimozione esplicita elimina prima i riferimenti Firestore e solo
dopo il blob privato: l'unica finestra di crash ammessa è quindi un orfano
owner-only. Il record chiuso `aiVisualRemovals` accetta solo identità e segmenti
autorevoli, UUID v4, `Timestamp` risolto e path canonico ricostruito; presenza
malformata fallisce prima dell'Admin SDK e non viene mai trattata come assenza o
riscritta. Consente un retry esatto e impedisce a un cleanup vecchio di cancellare un rimpiazzo. L'abbandono elimina il ticket e
...
immagine è `subject`; preset, preambolo e allowlist derivata restano server-side. Dopo il bind,
...
VE-05A. Gli holdout A, B e C non hanno superato il gate e sono archiviati: le
loro fonti non possono essere riutilizzate per validare le correzioni. Il dataset v4
contiene un holdout D indipendente. Un fallimento `pre_invocation` dimostra zero
provider call e viene contabilizzato a costo zero; soltanto un esito con rischio
di fatturazione lascia il totale reale sconosciuto.
