...
**M5-QUALITY-07** estende l'allowlist runtime, senza attivare il provider, a **due** coppie modello→listino univoche: `gpt-5.4-nano-2026-03-17`→`v2-2026-07-17-hg-m5` e `gpt-5.6-luna`→`v5-2026-07-20-luna-dev` (1.000.000/6.000.000 micro-USD per 1M input/output; cached non conteggiato). L'accoppiamento è autoritativo e fail-closed: modello non ammesso, listino non accoppiato (Luna con listino nano, nano con listino Luna) o versione sconosciuta ⇒ provider disabilitato prima di leggere il secret, costruire il grader o prenotare budget. Nessun fallback automatico tra modelli; nano resta scelta esplicita e nulla cambia automaticamente in `settings/aiConfig`, `OPENAI_PRODUCTION_MODEL`, Firestore o Secret Manager. Le Rules/Storage Rules/indici **non** sono modificati. `aiCorrectionRuns` registra il `model`/`priceListVersion` usati, senza UID/ID applicativi/contenuti; invariante `costActual ≤ costSettled ≤ costReservation` verificata con Luna e Unicode. Rollback immediato via `enabled=false` (kill switch senza deploy), poi ritorno esplicito a nano. Provider reale non attivato in questa change: zero chiamate, costi o deploy; G7 aperto fino allo smoke reale DEV.
