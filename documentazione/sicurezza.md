...
- Da M3-lite, l'identità dello studente è l'account Google usato per il login: SchoolForge non raccoglie né richiede nome, cognome, classe o altri dati autodichiarati per accedere in lettura al Portale. Non raccoglie email (Firebase Authentication gestisce l'identità, l'applicazione non ne fa un uso proprio oltre alla risoluzione del ruolo).
...
| G4-lite | Login Google risolve correttamente TeacherShell/StudentShell; nessun accesso anonimo; studente legge solo `publicLessons` e verifiche `attiva`+`public`; pool, soluzioni, `questionIndex` e documenti tecnici mai raggiungibili dallo studente; PDF studente senza soluzioni; nessuna Cloud Function introdotta. |
