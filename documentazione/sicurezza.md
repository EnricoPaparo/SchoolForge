...
**Versione:** 3.0
**Stato:** in vigore — controlli implementati da F-04 in avanti (M1, M2, M3-lite, RE, M3-full); M3-full completato con Gate G5 superato (vedi `documentazione/evidenze/g5-m3-full-checklist-finale.md`); M4-00 (contratto dati correzione), M4-01 (Security Rules + service layer `corrections`/`correctionEvents`/`correctionReturns`, vedi §2 sotto), M4-02 (workspace docente di correzione) e M4-02B (lettura studente della correzione restituita, sola query `correctionReturns` — vedi §2 sotto) implementati — Registro Correzioni/export (M4-03) e M5 restano non implementati/rinviati
...
| G6 (M4, M4-00/M4-01/M4-02/M4-02B implementati — Registro Correzioni/export non ancora) | Correzione, audit, eliminazione ed export solo docente; export non persistito; richiede G5 (M3-full, superato). Il contratto dati (`CorrectionDoc`/`CorrectionEventDoc`/`CorrectionReturnDoc`) è definito in `api-contract.md`; Security Rules e service layer sono implementati (M4-01, `pnpm test:rules` verde); il workspace docente di correzione (M4-02) e la lettura studente della correzione restituita (M4-02B) sono implementati; Registro Correzioni ed export (M4-03) restano non ancora implementati. |
