...
describe('Firestore rules — /verifications/{verificationId} delete', () => {
  ...
  it('owner can delete a draft verification', async () => {
    await seedOwner();
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
      await setDoc(doc(ctx.firestore(), 'verifications/v1'), DRAFT_DOC);
    });
  ...
  it('non-owner cannot delete a draft verification', async () => {
    await seedOwner();
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
      await setDoc(doc(ctx.firestore(), 'verifications/v1'), DRAFT_DOC);
    });
  ...
  it('unauthenticated user cannot delete a draft verification', async () => {
    await seedOwner();
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
      await setDoc(doc(ctx.firestore(), 'verifications/v1'), DRAFT_DOC);
    });
